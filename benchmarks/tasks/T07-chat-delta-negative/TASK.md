# T07-chat-delta-negative — Chat-level delta (NEGATIVE control)

**Scenario (canonical definition):** `../SCENARIOS.md` §T07 — state fixed there; both arms trace the SAME state.
Free-text 'tambah kolom npwp di form nasabah'; NO cheap delta lane exists in either arm -> full re-vault + re-bind. Expected ~0 structural delta; documents what P1-P4 did NOT fix (see proposals doc).

Maps to: T08 Cross-Module Feature entered as a chat ticket.
