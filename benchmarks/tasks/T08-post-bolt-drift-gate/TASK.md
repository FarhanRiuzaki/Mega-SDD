# T08-post-bolt-drift-gate — Post-bolt drift gate (TRUE control)

**Scenario (canonical definition):** `../SCENARIOS.md` §T08 — state fixed there; both arms trace the SAME state.
detect-drift standalone gate after a bolt, zero drift. detect-drift's loading was untouched by P1-P4; expected ~0 delta (file-diet residue only).

Maps to: T06 Refactoring safety-check. CONTROL.
