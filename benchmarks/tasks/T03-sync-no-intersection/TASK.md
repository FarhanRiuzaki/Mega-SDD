# T03-sync-no-intersection — Sync with ZERO claim intersection

**Scenario (canonical definition):** `../SCENARIOS.md` §T03 — state fixed there; both arms trace the SAME state.
Teammate hotfix changed 2 files intersecting no binding claim/unit target; `/mega-sdd:sync`. Optimized arm short-circuits via scripts/sync-intersect.sh (exit 0); baseline runs the full Mode D reconcile.

Maps to: T04 Bug Fix (hotfix landed outside spec surface).
