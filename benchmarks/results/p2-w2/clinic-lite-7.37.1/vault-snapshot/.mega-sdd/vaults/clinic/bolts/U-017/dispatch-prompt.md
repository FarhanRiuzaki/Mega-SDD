═══════════════════════════════════════════
BOLT SUBAGENT DISPATCH — U-017
═══════════════════════════════════════════
mega-sdd-trace:execute-bolts:U-017
...
1. Create the client component `ReassignDialog` with props `{ open: boolean; appointment: IAppointment | null; onClose(): void }`, rendering an MUI `Dialog` with the appointment summary (patient name, current doctor, date/time), a labelled select of `useDoctors()` minus `appointment.doctorId`, and the note that the patient will be notified by email.
2. The confirm button is disabled until a doctor is chosen and while the mutation is pending; it calls `useReassignAppointment` with `{ id: appointment.id, doctorId }`, closes on success, and on failure keeps the dialog open with the error message as text.
3. Write `index.test.tsx` (MSW `server.use` for doctors and the reassign route) with the three tests named exactly as the acceptance_test `expects` strings, including the adversarial check that the POST body contains the chosen `doctorId` and the dialog stays open on a 400.
...
(selector: `\b[A-F]-\d{3}\b` cited in U-017.md OUTSIDE code fences/spans, AND resolving to a real clause block in constitution.md; binding claim ids and retired clauses excluded)
...
```
### T2 budget tracker
consumed_t1: 7341 bytes (cap 12288)
consumed_t2: 10064 bytes (cap 10240, hard 12288)
total: 17405 bytes  # T1 + T2 ONLY — the budgeted, truncatable content
file_total: 22560    bytes  # THIS WHOLE FILE; the gap from `total` is the four
                            # un-budgeted, never-truncated blocks (TIER 2 banner,
                            # this tracker, TIER 3 list, PROVENANCE appendix).
truncations_applied:
  - validation_hints: drop expected-output patterns; keep test commands only (saved 241 bytes)
  - validation_hints: drop section entirely (drop floor) (saved 405 bytes)
...
- depends_on_summaries: every upstream in depends_on lacks a readable bolt-report.md
