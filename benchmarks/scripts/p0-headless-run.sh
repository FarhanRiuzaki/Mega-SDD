...
#
# usage: p0-headless-run.sh <arm-dir> <prd-rel-path> <log-dir> [model]
#   prints the session id + transcript path; runs detached (nohup); exit 0 = launched.
set -u
ARM="${1:?arm dir}"; PRD="${2:?prd rel path}"; LOG="${3:?log dir}"; MODEL="${4:-opus}"
...
PROMPT="jalankan mega-sdd dari $PRD sampai semua unit selesai (DONE), lalu jalankan /mega-sdd:analyze di akhir."
...
{
  echo "sid=$SID"; echo "arm=$ARM"; echo "prd=$PRD"; echo "model=$MODEL"; echo "transcript=$TRANSCRIPT"
