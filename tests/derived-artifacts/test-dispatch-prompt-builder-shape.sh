...
PY
$PY - "$WORK/ui.json" <<'PY' && ok "C3: inline_core carries the literal mega-sdd-trace:execute-bolts:<unit-id> line (v7.3.1 gateway contract)" || fail "C3: gateway tag missing/misspelled"
import json, sys
d = json.load(open(sys.argv[1]))
...
$PY - "$WORK/inline.json" <<'PY' && ok "C6b: whitelist degraded to count+pointer, still <=700B, tag + abs path never dropped" || fail "C6b: inline_core degradation path wrong"
import json, sys
d = json.load(open(sys.argv[1]))
core = d["inline_core"]
...
$PY - "$WORK/def.json" <<'PY' && ok "J3: ...and still emits a capped inline_core with the gateway tag" || fail "J3: inline_core broken on the default-plugin-root path"
import json, sys
d = json.load(open(sys.argv[1]))
