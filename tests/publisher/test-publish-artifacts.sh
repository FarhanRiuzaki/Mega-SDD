...
assert m["project_id"] == "git.example.com/grup/repo", m["project_id"]
...
#   "env": { "ANTHROPIC_BASE_URL": "http://192.0.2.20:8001" }
...
( cd "$P5" && git init -q && git config user.email t@t && git config user.name t \
  && git remote add origin https://git.example.com/grup/winrepo.git \
  && git add -A && git commit -qm init )

echo "── w1: quoted-absolute .cmd helper + http URL arms the office rung ──"
echo 200 > "$LOG/next-response"; echo 0 > "$LOG/count"
clean HOME="$HOME_WIN" USERPROFILE="$HOME_WIN" ANTHROPIC_BASE_URL="http://192.0.2.20:8001" \
  PATH="$SHIM:$PATH" bash "$SCRIPT" --cwd="$P5" </dev/null >/dev/null 2>&1
[ "$(cat "$LOG/count")" = "1" ] && grep -q '192.0.2.20:8001/mega-sdd/ingest' "$LOG/last-argv" \
...
( cd "$P3" && git init -q && git config user.email t@t && git config user.name t \
  && git remote add origin https://git.example.com/grup/scanonly.git \
  && git add -A && git commit -qm init )
...
assert m["project_id"] == "git.example.com/grup/scanonly", m["project_id"]
