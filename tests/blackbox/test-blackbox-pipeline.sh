...
write_binding 1
OUT="$(bash "$SCR/stamp-binding-boilerplate.sh" --vault "$VAULT" </dev/null 2>&1)"; RC=$?
[ $RC -eq 0 ] && ok "stamp rc=0" || bad "stamp rc=$RC: $OUT"
...
[ $RC -eq 0 ] && ok "derive-binding-json rc=0" || bad "derive-binding-json rc=$RC: $OUT"
...
write_binding 2
bash "$SCR/stamp-binding-boilerplate.sh" --vault "$VAULT" </dev/null >/dev/null 2>&1
