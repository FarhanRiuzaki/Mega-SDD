#!/usr/bin/env bash
# test-dep-authorization.sh — execute-bolts code gate 6 (P9, v5.3.0; v7 Fase 2
# group 3: the check now rides validate-new-deps.sh --unit= as the
# `authorization` JSON section — one manifest-diff pass with gate 5).
...
# a unit with no allowed_new_deps key is a no-op. The authorization concern
# NEVER affects the exit code (gate-5 registry semantics own it) — the arms
# below force the registry offline (unroutable proxy → 'unverified', exit 0)
# so the advisory posture is still observable deterministically.
set -u
HERE="$(cd "$(dirname "$0")" && pwd)"
ROOT="$HERE/../.."
SC="$ROOT/plugins/mega-sdd/scripts/validate-new-deps.sh"
# deterministic offline: registry lookups fail instantly -> status 'unverified'
export http_proxy="http://127.0.0.1:9" https_proxy="http://127.0.0.1:9"
run_auth() { bash "$SC" --unit="$1" --base="$2" --head="$3" --cwd="$4" 2>/dev/null \
  | python3 -c 'import json,sys; print(json.dumps(json.load(sys.stdin).get("authorization") or {}))'; }
...
printf -- '---\nid: U-1\n---\n# u\n' > "$R/u/absent.md"
OUT="$(run_auth "$R/u/absent.md" HEAD~1 HEAD "$R")"; E=$?
...
printf -- '---\nid: U-1\nallowed_new_deps: []\n---\n# u\n' > "$R/u/empty.md"
OUT="$(run_auth "$R/u/empty.md" HEAD~1 HEAD "$R")"
...
OUT="$(run_auth "$R/u/inline.md" HEAD~1 HEAD "$R")"
...
printf -- '---\nid: U-1\nallowed_new_deps:\n  - axios\n  - fakepkg9zz\n---\n# u\n' > "$R/u/block.md"
OUT="$(run_auth "$R/u/block.md" HEAD~1 HEAD "$R")"
...
bash "$SC" --unit="$R/u/empty.md" --base=HEAD~1 --head=HEAD --cwd="$R" >/dev/null 2>&1; E=$?
[ "$E" -eq 0 ] && ok "unauthorized finding STILL exits 0 offline (advisory-first; exit code is gate-5's alone)" || bad "advisory exit $E"
OUT="$(run_auth "$R/u/empty.md" HEAD~1 HEAD "$R")"
...
# ── usage guard (merged script = gate-5 semantics: missing base/head -> usage exit 2) ──
bash "$SC" >/dev/null 2>&1; [ $? -eq 2 ] && ok "missing args -> usage exit 2 (gate-5 contract owns the exit code)" || bad "usage should exit 2"
