...
for f in "$CONV"/*.md; do
  ...
  printf '%s\n' "$body" | grep -q -- '<[A-Za-z]' && fail "$b: unfilled <placeholder> left in the section"
...
[ "$n" -ge 1 ] && pass "$n pack(s) carry the section" || fail "no pack carries the section"
