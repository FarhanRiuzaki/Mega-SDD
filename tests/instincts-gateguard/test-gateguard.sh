...
# v7 (spec 2026-08-21 §3.1): GateGuard is chain-scoped — arm the session the way a
# real chain does (a mega-sdd:* Skill dispatch writes chain_engaged). Arming a NEW
# session also resets the dedup entries — exactly the cross-session semantics below.
arm() { printf '{"session_id":"%s","cwd":"%s","tool_name":"Skill","tool_input":{"skill":"mega-sdd:memory","args":""}}' "$1" "$T" | bash "$H" >/dev/null 2>&1; }
...
# different session -> gates again (the new session arms on ITS first dispatch,
# which resets the session-keyed state — fresh first-touch deny)
arm s2
