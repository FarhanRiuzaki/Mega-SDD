...
| A5 | ~~agent_type-scoped PreToolUse fast path~~ | hook inputs `agent_id`/`agent_type` (v2.1.229) | **REJECTED on measurement 2026-08-17** (`research/2026-08-17-a5-agent-scoped-gate-verdict.md`): the premise was false — the heavy aggregator is Skill-scoped and lenses never hit it; what they pay is 3 spawns/call of which the parse is unavoidable and the anti-self-bypass battery is exactly what a Bash-capable agent must keep | — |
...
| A8 | ~~`FileChanged` sync probe~~ | new hook event (v2.1.229) | **REJECTED on doc evidence 2026-08-17** (spec 2026-08-17-a7-userconfig.md): matcher is LITERAL filenames, not globs — the third-channel use is unsupported; vault-artifact watching duplicates gate re-derivation | — |
