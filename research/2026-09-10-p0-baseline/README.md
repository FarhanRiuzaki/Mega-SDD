...
| clinic | **7.35.0** (headless, attempt 2; attempt 1 di 7.34.0 = bukan data, deadlock handoff — `§3c`) | opus | 1h19m35s → ep1 · 4h34m44s → DONE | 0 (headless; 32,7 m "USER"/IDLE = artefak wave paralel) | 1h16m01s | 0 (headless) | **1h00m33s** | 15m27s | **79,7 %** | pra-kode 1h00m · DONE 4h35m / ≤2h MISS | 0 ask + 18 `[ASSUMED-BY-RUNNER]` (≤3 n/a) | 36 (21 unit + 4 gate re-dispatch + 11 fix round) | 4 lens ×21 | — | — | — | — |

Transkrip + `git log` yang dipakai: `benchmarks/results/p0-baseline/<arm>/run.meta` (transcript=) + `git-log.txt`; ekstraksi = `benchmarks/scripts/p0-extract-arm.sh <dir>`. **Verdict kill-criterion: LANJUT P2** (xs 80,1 %, klinik 79,7 %) — `research/2026-09-10-v8-p0-baseline.md §5`.
