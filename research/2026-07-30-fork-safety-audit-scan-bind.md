...
**Path prefix:** `PLUGIN` = `/Users/<user>/SunnyGo/2026/AIRND2026/Project/mega-sdd-github/plugins/mega-sdd`, `REPO` = `/Users/<user>/SunnyGo/2026/AIRND2026/Project/mega-sdd-github`. Every path below expands from one of those two.
...
> **SUPERSEDED 2026-07-30 (same day, adversarial re-review) — do NOT implement (A) as written.** (A) shipped and was a **live regression**: the pre-existing `--auto` behaviour *proceeded*, and per this very document's own monorepo finding **zero** routing rows pre-resolve `--engine`/`--include` — nor do they render `--auto` on the *scan* hop — so an unconditional blocker hard-stopped phase 1 of nearly every brownfield chain (on Windows the gate trips at ~272 files). The shipped decision is a **three-lane split**: lane 1 decided (explicit `--engine=`/`--include=`) → proceed; lane 2 undecided STANDALONE → the (A) blocker; lane 3 UNATTENDED (`--auto` / forked / orchestrator-dispatched, ties resolved here) → (B)'s loud recorded downgrade, with the `scan-procedure.md` reconciliation written out in full. Authoritative record: `docs/superpowers/specs/2026-07-30-token-and-latency-optimization.md` §"5a amendment" → Decisions taken → Spawn-cost gate. **B2 / B4 / B6 below must be read against that decision, not against (A).**
