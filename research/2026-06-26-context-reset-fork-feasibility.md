...
## In-environment validation (2026-06-26) — detect-drift fork, behavior-faithful

A `context: fork` skill *is* its body run as a subagent prompt with no conversation history. That runtime condition was reproduced faithfully by dispatching the **exact branch `SKILL.md` body + references** as a default (general-purpose) subagent against a purpose-built `mode=existing` vault + a Laravel codebase with 5 planted drift items — **no behavioral coaching** in the wrapper (so the skill prose, not the harness instructions, had to carry non-interactivity). Two paths, both graded on disk (not on the agent's self-report):

- **Happy path** — wrote `DRIFT-REPORT.md` (decision findings PRIORITY-1 at top), a genuine `PENDING-SYNC.md` **queue** ("detect-drift does not resolve them → resolve via resolve-oq/sync", direction-neutral, fingerprinted, "nothing written back" with no `--auto-apply=safe`), and `.memory/drift-history.md` **persisted to disk**; vault integrity preserved (`vault.json` untouched, version stayed 1.2, session-only Changelog, no `DRIFT-ACTIONS.md`). All HIGH-confidence plants caught (name-drift `approved_by`→`reviewer_id`, `notifications` + `priority` missing-in-vault) plus the D-002 violation (the skill's own lowest-confidence axis). **No `AskUserQuestion`** — confirmed absent from the fork toolset.
- **Unresolvable `--code`** — short-circuited at Step 0 with the `drift_inputs_missing(code)` blocker, **zero files written**, no prompt, no improvised substitute repo. The cleanest discriminator: the prose's deterministic-input discipline held under fork.

**What this proves / what it doesn't.** Green = the forked skill *behaves* correctly and runs in a *fresh small context*. It does NOT exercise the harness parsing `context: fork`, nor the before/after token delta — those need a live pipeline run on the user's machine. Standalone footprint of the happy-path fork: 2.4M raw / **~605K cost-weighted across 45 turns** (a fresh context that inherits none of the main pipeline's accumulated history — the saving mechanism). Side finding that validates Batch 1: the harness's own `subagent_tokens` reported only the **last turn** — undercounting the true summed cost by **8.2× (cost) / 32.8× (raw)** — which is exactly why the SubagentStop hook sums `message.usage` across every turn and cost-weights it. Deterministic contract pinned by `tests/drift/test-detect-drift-fork.sh`; this runtime check is supplementary evidence, not a CI gate (it needs a live agent dispatch).
