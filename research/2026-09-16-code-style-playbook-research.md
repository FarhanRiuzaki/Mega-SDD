...
3. **Nilai tajam playbook per stack = daftar toolchain yang MEMBACA doc-comment di stack itu.** Aturan generik "docblock hanya bila ada toolchain yang membacanya" (Rule 6) tidak menyebut toolchain mana — di Go (`revive exported`), Ruby (RuboCop `Style/Documentation` — **KOREKSI 2026-09-16 malam:** cop ini `Enabled: false` di default config RuboCop saat ini, hanya wajib bila `.rubocop.yml` proyek mengaktifkannya), C# (`CS1591` + Swashbuckle), FastAPI (docstring → OpenAPI) doc-comment justru **diwajibkan/dikonsumsi**, jadi implementer yang terlalu patuh pada "jangan komentar" bisa merahkan gate lint L0 → fix round. Playbook per stack mencegah kolisi itu; itu alasan non-gimmick-nya.
...
| Ruby — `rails`, `sinatra` | YARD / RDoc | RuboCop `Style/Documentation` — **dikoreksi (verifikasi `config/default.yml` 2026-09-16): `Enabled: false` di default RuboCop saat ini**; wajib hanya bila `.rubocop.yml` proyek mengaktifkannya — lalu class/module top-level butuh komentar satu baris | `.rubocop.yml` (`:93-94`) | sedang — tinggi hanya di proyek yang mengaktifkan cop-nya |
