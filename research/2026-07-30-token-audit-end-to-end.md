...
| 2 | Bolt dispatch prompt (~9KB/bolt) is model-assembled from pure copy/filter/sort/cap logic **and materialized twice** | **gross 19.9–59.7 min**, **net ~15–55 min** of serial main-thread generation for a 40-unit run (MEASURED 2026-07-31: builder median **0.186 s/bolt** macOS, n = 140; **DERIVED** ~1.98 s/bolt Windows-EDR from 6 measured exec'd binaries + ~3 subshell forks × 220 ms; plus one new subagent Read round trip, **assumed** 1–3 min/run and the only unmeasured term). The previously published "gross 25–75 min" is struck — it does not reproduce from the confirmed 179K at the 50–150 tok/s band the same sentence states. | **helps strongly** — **MEASURED 2026-07-31 against the shipped working tree; n = 25 non-UI / 10 starterkit-UI / 10 greenfield-UI real units, 105 runs, all exit 0.** OLD side ~179K output tok/run CONFIRMED and unaffected. NEW side, enumerated (Bash command + Bash description + `inline_core` + `subagent_type` + Agent description): **578 B–1,251 B per bolt = 5,780–12,510 output tok/run ⇒ 14.3×–31.0×** (centre 17.4×–24.2×), same 4 chars/token divisor both sides, and a **lower bound** because the new side counts tool `description` params the 179K side never did. Both prior figures (~4.8K / 37.3×, ~6.9K / ~26×) are **struck** — each was re-derived from the previous round rather than from shipped code. **Separate line items, never folded into the ratio:** (i) the builder's non-suppressible stdout, three figures that must never be averaged, at this repo's own 67-byte root — non-UI **731/815/922 B**, starterkit-UI **985/1,043/1,343 B**, greenfield-UI **1,199/1,256/1,343 B** (8,150–9,252 input tok/run; +3 B/bolt per character of project-root length); (ii) the design-lens hand-off, which the round-3 D3 change turned from a verbatim slice paste into one path — **9,506 B ≈ 2,377 output tok saved per greenfield UI bolt** (3,291 B when the design_slice truncation rung fires; only 366 B on a starterkit repo). No cost-weighted ratio is claimed in either direction — the OLD flow's input debit was never quantified. See `docs/superpowers/specs/2026-07-30-token-and-latency-optimization.md` §2b. |
