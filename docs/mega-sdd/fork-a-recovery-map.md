...
**(2) Gate-time re-derivation** — the PreToolUse gate re-runs the validator on every guarded dispatch (a stale/forged/absent state file is overwritten before it is read); advisory validators are re-run by `analyze` FULL. (The v7.5.0 diet deleted the PostToolUse fan-out.)
