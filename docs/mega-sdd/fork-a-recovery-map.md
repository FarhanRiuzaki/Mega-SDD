...
| **Deep extraction disciplines (P1–P4)** | [SKILL-PROSE] wired to dispatch | **✅ Wiring shipped v3.72.0; reasoning Fork-B real-run** | P1 writer↔reader provenance + clone-inheritance, P2 enumerate-all-sites + entry-points, P3 behaviour-as-executed, P4 structural classification — placed in `agents/domain-extractor.md` §Deep disciplines (the subagent's system prompt — received by every wave subagent by construction; moved there from `wave-dispatch-templates.md` in tranche 5d) + design-vocabulary in `extract-intelligence/SKILL.md`. WIRING presence is fixture-asserted; the LLM actually performing the reasoning is best-effort Fork-B (real-run-only). Wave-3 advisory `provenance_read_side_thin` (non-blocking, never flips status). |
