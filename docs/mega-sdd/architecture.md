...
- **Output:** 4-file layout-2 vault (`vault.md` / `model.md` / `flows.md` / `constraints.md`) + `vault.json` on the classic lane; layout-3 `context.md` + `constitution.md` + `vault.json` when written by `plan` (`--lite`) or migrated with `migrate-paths --vault-layout=3`; legacy 7-file vaults are read-only
...
- **Input:** vault (+ the symbol index; `codebase-map.md` on the `--classic` spine)
- **Output:** `<vault>/bound/` + `binding.md` (classic); per-unit `bolts/U-XXX/binding.json` on `--lite`
...
- **Repo access:** Write
- **Rails:** TDD (superpowers optional), target_files enforcement, halt-on-failure after max retries
