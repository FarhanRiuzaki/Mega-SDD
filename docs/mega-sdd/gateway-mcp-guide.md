...
> **Amandemen v7.3.0/v7.5.0 (baca dulu):** `mega-sdd-trace:session` SUDAH TIDAK ADA — di-retire v7.3.0 dan sengaja TIDAK dikembalikan di v7.3.1 (dipin NEGATIF oleh `tests/hooks/session-start.test.sh`); deteksi sesi sepenuhnya urusan gateway, diturunkan dari `:turn`/`:<skill>`. Tag `:turn` TIDAK punya opt-out config (`trace_tag` tidak pernah ada di kode; kontrak, bukan preferensi). Dan sejak v7.5.0, hook yang sama boleh menambahkan SATU baris kedua non-tag (tawaran sync dari census "selesai") — **filter WAJIB key ke BARIS PERTAMA, byte-verbatim**; namespace `mega-sdd-trace` tetap eksklusif untuk tag. Kontrak penuh: `docs/gateway-contract.md`.

| Marker | Muncul di | Kapan | Opt-out yang mempengaruhi |
|---|---|---|---|
| `mega-sdd-trace:turn` | BARIS PERTAMA output UserPromptSubmit, tiap prompt | hanya di project `.mega-sdd` (CWD non-SDD = hening total) | tidak ada — unconditional di project ter-adopsi |
...
Aturan pakai: **hard check (warn→block) keyed ke `mega-sdd-trace:turn`** (+ `:<skill>` untuk breakdown per-skill), dievaluasi AGREGAT per NIP per window — sesi di CWD non-SDD memang hening by design, perlakukan netral, bukan pelanggaran; `plugin_version` untuk version-floor report. String marker adalah KONTRAK STABIL dari sisi plugin (dipin `tests/weighted-routing/test-tier-s-hooks.sh` + goldens dispatch-parity); jangan regex-longgar — match verbatim baris pertama cukup dan murah.

**Nuansa sidechain (WAJIB dipahami sebelum mode block):** subagent berjalan di konteks FRESH — request-nya TIDAK membawa history, jadi TIDAK membawa tag `:turn` milik parent. Subagent yang di-dispatch pipeline mega-sdd tetap terdeteksi karena markernya DITANAM deterministik di dispatch prompt (`mega-sdd-trace:execute-bolts:<unit_id>` di-generate `build-dispatch-prompt.sh`, golden-pinned; skill lain via template dispatch) — tapi subagent generik non-mega-sdd (Explore/Agent bawaan Claude Code, plugin lain) sah dan TIDAK bermarker. Konsekuensi: **block per-REQUEST akan membunuh sidechain traffic yang sah — jangan.** Bentuk yang benar: keputusan AGREGAT per NIP per window (mis. 15–30 menit): kalau SELURUH traffic Claude Code sebuah NIP dalam window tidak memuat satu pun `:turn`/`:<skill>` → warn/block NIP tersebut; request sidechain tanpa marker dalam window yang punya tag = netral, bukan pelanggaran.
