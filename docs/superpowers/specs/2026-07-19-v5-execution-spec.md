...
| ~~P8~~ | ~~5.2.0~~ | **DEFERRED (measured, `research/2026-07-20-p8-terse-plane-assessment.md`; user-confirmed "narrow safe-terse later P10").** The emitters flatten+sha-stamp vault prose as the human docs' cited source → terse-ing it breaks invariant-3 citation AND degrades the human docs (counter to the 3-human-docs goal); the synthesis-switch substitute reintroduces fabrication risk (invariant #5). Resident 0.1× lever anyway. A narrow safe-terse (cut only regions no gate parses AND no emitter cites) rides P10, telemetry-gated. | — |
| P9 | **5.2.0** | **dep-authorization ONLY** (`allowed_new_deps` allowlist + `check-dep-authorization.sh` gate 6 + shared `_lib/dep_manifest.py`). The **reuse gate was NOT built** — reuse/duplication is already deliberately ADVISORY (test blocks wiring it into PreToolUse); escalating it would relitigate a settled gates-doctrine call. `DO_NOT_ADD_DEPS` (blocking) already exists; gate 6 is the graduated advisory allowlist complement (deterministic-first). | advisory-first (always exit 0), commit-keyed blocking deferred; absent key = no-op for pre-v5 units |
| P10 | — | **FIELD-DATA-GATED (v5 CLOSED at 5.2.0, user-confirmed 2026-07-20).** Every P10 item needs REAL field data to be worth shipping, so it rides that data, not the calendar: cost-weighted telemetry table ← real runs (fork-AB/MSmile — the instrument `seeding_budget.py --weight` + `measure-seeds.sh --json` already exists, this is wiring); alias-cull decision ← real usage telemetry (decision 2); narrow safe-terse ← real `.citation-map.json` proving what is uncited (P8 assessment); FSD §9 slim + SIT pointer ← rides the next FSD touch; optional `uat-results.yaml` ← field demand (decision 5). | telemetry decides, not assumption |

## v5 close-out (2026-07-20)

**The ladder closed at 5.2.0.** Three genuine increments realized the thesis — 5.0.0 (surface collapse, −48% always-on), 5.1.1 (advisor-bundle: the one real fresh-1.0× seed lever), 5.2.0 (dep-authorization: the one missing WAJIB mechanism) — all CI-green. Four consecutive phases (P7–P9) measured smaller than specced under the discipline this spec's own guards demanded (cache lens, reuse-first survey, pin-audit): the specced work was repeatedly found already-built, resident-cheap, self-defeating, or settled-by-design. That is a finding about the codebase's maturity, not a failure of the spec: the earlier god-review/token-efficiency/P4 work had already banked most of the ladder. Measurement records: `research/2026-07-19-p7-seed-budget-baseline.md`, `research/2026-07-20-p8-terse-plane-assessment.md`.
