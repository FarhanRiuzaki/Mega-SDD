...
| Stop bootstrap on a reftable repository (8.8.1) | the leg reads the sha the turn-gate probe of the same hook already paid for (`rev-parse`, 1 process) when the builtin reader cannot read reftable; files-backend and unborn repositories are unchanged | 8.8.0 took reftable for an unborn repository and never bootstrapped, so a tier-S-only reftable machine kept the "not computed yet" header until GROUND; measured after the fix: cache written, SessionStart hit 2 / miss 3 (the §10 DERIVED cells) |
...
**Measured (production dispatch, PATH shim, macOS):** C10 derive-unit-claims 4 → 5; C11 write-unit-binding 4 → 5; C8b the in-run gate on its ALLOW path 87 → 89 (+2 git, the `dirty_map`; the leg rides the aggregator's python). No ceiling was raised. C11b, the writer's conditional paths (a re-bind after the unit's own bolt: rung 3 + `absent_at` + ordering + carry-forward + the authoring snapshot), measures **20** (git 17) against §10's DERIVED maximum of 18 (steady 6 + conditional 12). The 2 over are the unbatched authoring snapshot (the row above), which also grows with the unit's commit count. Per D30 this number goes to the owner; the pin is the measured value, not a raised C11 ceiling.
...
**§13 coverage audit (8.8.1).** Every §13 bullet was mapped to an existing assertion, and the unmapped ones were written:
- the gate suite gained Bash-shaped writes to `binding.json` (python open-write, `sed -i`, `cp`, `mv`, `tee`, redirect) and a read that stays open; skip-worktree and gitignored exact scope files at bind (ALLOW, then a later change to the ignored file DENYs); the C2 edit staged after 3.9 (DENY); dirt outside the unit scope (ALLOW, stamp not nulled); the two-wave run, with a no-step-0 control arm that shows the index really was stale;
- the spawn suite gained C11b, the reftable pins (C21) and the no-python bootstrap pin (C22);
- covered differently than written: the C1 `BASHPID` process pin is the static fork pin in `tests/hooks/ups-head-move.test.sh`; the identity cases of the "extended `agent-dispatch-gate.test.sh`" live in `tests/state-anchor/test-gate-binding-freshness.sh`.

**Open for the owner (found by the audit, deliberately not changed).** An in-range anchor on a file that git never tracked (gitignored or untracked) CONFLICTs `anchor_content_drift` on its FIRST bind: the unit file's authoring snapshot exists, but the anchored file is absent from it, so the ladder has no reference, and the printed reason ("changed since it was bound") is not true. A candidate rule: when the file is absent from the snapshot AND untracked at HEAD, the current block is the reference (as for an anchor authored against the working tree), at +1 exec on that rare path only. The wider class also covers a file committed after the unit was authored. The change moves a verdict from CONFLICT to CONFIRMED (invariant #2) in a case this spec does not cover, so it waits for the owner; today it fails safe (one `--resolve=KEEP_CODE`, carried forward).
