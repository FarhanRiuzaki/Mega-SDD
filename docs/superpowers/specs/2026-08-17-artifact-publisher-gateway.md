...
3. **Config** — `.mega-sdd/config.yaml` `publish.gateway_url` + `publish.token` (seeded via plugin `userConfig`, token `sensitive: true`). Opt-in per project; token never logged.
