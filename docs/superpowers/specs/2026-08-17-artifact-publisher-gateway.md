...
3. **Credentials — mega-code first (office reality, corrected 2026-08-17: the team runs Claude Code DIRECTLY with TWO plugins — `mega-code` as the LOGIN plugin (npm-distributed; dev logs in with their NIP) and `mega-sdd` as the SDD plugin; mega-code does NOT launch claude, so it cannot set process env).** Publisher probe ladder, first hit wins: (1) env `MEGA_SDD_PUBLISH_URL`/`MEGA_SDD_PUBLISH_TOKEN` — a GENERIC override for CI/testing, not a mega-code channel; (2) **the mega-code channel: credential file `~/.mega-code/megasdd-publish.json`** (`{"gateway_url":…, "token":…}`, permission 600 — written by mega-code's login flow from within the session or its CLI); (3) generic fallback `.mega-sdd/config.yaml` `publish.*` / plugin `userConfig` (non-office users). No hit → publisher inert. Token never logged; 401 → fail-open + queue + one log line "re-run mega-code login" (rotation/expiry is mega-code's job). **No PII in artifacts:** the manifest carries no NIP — the gateway attributes the push from the token it minted per NIP at login.
