...
4. **(Round adversarial) `engaged_sessions` map — arming per-sesi, bukan single-owner.** Panel review menemukan regresi shared-worktree (workflow nyata user): sesi B yang dispatch skill mega-sdd me-restamp `.gateguard-state.json` dan MELUCUTI sesi A yang sedang chain. Fix: marker writer memelihara map `engaged_sessions: {sid: true}` (cap 50) di file yang sama; armed check membaca entry sid sendiri; GateGuard writer mempertahankan map lintas reset sesi. `.gateguard-state.json` juga MASUK daftar protected FP_GUARD + Bash anti-self-bypass (arm switch = kelas anti-forge; penulis sahnya hanya python internal hook).
5. **(Round adversarial) Derivasi handoff gate-time = absent-only di Fase 1.** Recompute tiap dispatch terbukti (a) memvalidasi teks turn berjalan (narasi ber-"handoff:" tanpa envelope → FAIL palsu yang mem-block dispatch yang pre-v7 lolos) dan (b) menggandakan retry_count pada teks yang sama (eskalasi C1→C2 prematur). Fase 1: gate-time hanya men-derive saat state ABSENT (menutup gap fresh-session/clone yang tidak pernah bisa ditulis Stop) DAN teks membawa envelope nyata (`emitted_by:` line-start); state yang ada tetap dipercaya (ditulis Stop di akhir turn — perilaku pre-v7). Recompute penuh tiap dispatch menyusul di Fase 2 bersama penghapusan leg Stop, dengan content-hash di state validator sebagai dedup retry. **[LANDED Fase 2 №2]** — Branch 1a kini recompute di SETIAP dispatch ter-guard saat last message membawa envelope nyata; `validate-handoff-yaml.sh` menyimpan `content_sha256` dan short-circuit idempotent pada teks identik (retry_count tidak dobel); leg Stop DIHAPUS di commit yang sama (gate-time = satu-satunya penulis).
6. **(Round adversarial) Robustness fast path:** regex config ter-anchor ke awal baris (kunci ter-comment tidak lagi match), size cap 256 KB (payload raksasa ambil jalur python klasik), stderr `$(<file)` ditekan via brace-group (bash 3.2).

7. **(Amendemen 7.29.1 — pencatatan pasca-7.3.x.)** `hooks/subagent-stop` (§3.6, §5 baris 7) dan marker `mega-sdd-trace:session` (§7.2) DIHAPUS di v7.3.0 (`bfdf996`, observability removal). v7.3.1 (`6caf4dc`) mengembalikan HANYA keluarga tag `mega-sdd-trace:turn` + `:<skill>` sebagai kontrak gateway (`docs/gateway-contract.md`) — bukan `:session`, bukan hook subagent-stop; hooks.json = 6 event. Baris-baris di atas yang menyebutnya = sejarah Fase 1.
