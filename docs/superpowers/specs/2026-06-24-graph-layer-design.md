...
- **Builder language — Bash wrapper + embedded Python3 (resolved; refined during planning).** The plugin's actual script house style is a Bash wrapper with `python3 <<'PYEOF'` heredocs (every `scripts/validate-*.sh`), using stdlib **+ PyYAML** — PyYAML is already a de-facto dependency of the validators, so it is not a *new* one. The graph builder follows this exactly: stdlib (`json`, `hashlib`, `glob`, `re`) **+ PyYAML with a stdlib regex fallback** for frontmatter (matching the plugin's tree-sitter→regex graceful-fallback doctrine). This supersedes the earlier "stdlib-only" wording while preserving the zero-*new*-dependency intent. Graph BFS + JSON emit + frontmatter parsing favor Python decisively over shell+`jq`/`yq`.
