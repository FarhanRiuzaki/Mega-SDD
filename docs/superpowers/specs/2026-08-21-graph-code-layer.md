...
"env": { "ANTHROPIC_BASE_URL": "http://<gateway-internal>:8001", "NO_PROXY": "<gateway-internal>,…" }
```
...
- Ingest URL for the field test: `http://<gateway-internal>:8001/mega-sdd/ingest`.
...
2. **`project_id` shape.** Runbook §2's example keeps the host (`git.example.com/grup/repo`); their §8 sed keeps the host for ssh form and **strips** it for https form — internally inconsistent. Ours keeps the host in both (matching their own §2 example). Since `project_id` is the store key, one rule must be agreed or a repo's artifacts split across two keys.
