...
> **Amendment (2026-07-07, token-efficiency B3/M-13b):** the SessionStart hook parses `source` from the captured stdin and sizes the anchor injection to it. **resume** → SKIP the anchor entirely (the reloaded transcript still contains a prior full injection, so re-injecting is pure F0 duplication; the dynamic state notices — staleness, COMPACT_RESUME, instincts, self-resolve — still fire). **compact** → inject only the SLIM core (Hard rule + Output language + a one-line "invoke using-mega-sdd for the rest" pointer), because compaction replaces context with a summary but the harness re-lists every skill description (the keyword census) and PreCompact already wrote COMPACT_RESUME. **startup | clear** → inject the FULL routing core (fresh/empty context). **Unparseable/absent source** → FAIL OPEN to the full core (never break routing to save tokens); a non-resume source whose extraction yields empty also falls back to the full skill. resume legitimately yields an empty anchor and is NOT overridden. Pinned by `tests/token-efficiency/test-b3-anchor-and-panel.sh` (per-source matrix) + the existing compaction pin.
