...
4. **Flag handling**:
   ...
   - `--express` — forwarded VERBATIM (orchestrate-flow owns it; the v6 P1 lane): the orchestrator appends `--express` to the `bind-codebase` hop it dispatches — bind then retrieves claim-scoped (script-derived claims ledger + symbol-index queries + targeted Reads, zero codebase-map load) with an honest fallback to the standard lane when the index/ledger is unavailable. No gate changes; verdict grammar identical.
