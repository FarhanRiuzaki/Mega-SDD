...
4. **Flag handling**:
   ...
   - `--express` / `--classic` — forwarded VERBATIM (orchestrate-flow owns them). **Express is the DEFAULT spine:** chains render without a scan phase (GROUND ran as a script at Lane step 1) and bind hops retrieve claim-scoped (script-derived claims ledger + model completeness sweep of the vault docs + symbol-index queries + targeted Reads, zero codebase-map load) with an honest fallback to the standard lane when the index/ledger is unavailable. `--classic` (or persistent `spine: classic` in `.mega-sdd/config.yaml`) restores the scan-first chains verbatim. No gate changes on either spine; verdict grammar identical.
