...
2. **Is `<input>` a path to a file?**
   - Extension `.{md,pdf,docx,txt}` → likely PRD. Propose chain starting with `generate-intent <input>` (Mode A). **Lite lane (`--lite` on the call, or `derived.lane: lite` from config) — v8 P2 2-hop chain:** the engine's `derived.proposed_next` renders `plan <input> --lite --mode=<existing|new>` → `execute-bolts --all --lite` (no generate-intent / bind / generate-units hops — JIT bind runs at dispatch); ONE confirmation covers both hops; `plan` emits NO handoff YAML — after it returns, re-run `derive-state.sh` and `validate-preflight.sh --predictive --cwd=<root> --chain=execute-bolts` (lite checks: units present + coverage PASS), then dispatch bolts. Classic (no lane) is byte-identical to today.
...
4. **Flag handling**:
   ...
   - `--model-tier=inherit|auto|haiku|sonnet|opus` / `--no-escalate` — forwarded VERBATIM to the `execute-bolts` hop (v7.1 per-unit implementer routing + cascade; execute-bolts §Flags owns the semantics; config default `model_tiers.bolt_implementer: inherit` = today's behavior).
...
- `--lite` — v8 P1 lane (opt-in; spec 2026-09-10 App. F8). **P2 2-hop lane:** with a PRD argument the proposed chain is `plan <prd> --lite --mode=…` → `execute-bolts --all --lite` (state re-derived from disk between the two hops; no handoff YAML). Also: execute-bolts pre-flight 3.9 JIT bind runs for EVERY wave (not only units carrying `## Claims`), the W1 zero-idle behaviors apply (batched ask absorbs L0, DEFER-class halts quarantine, exactly two interaction points on a happy path), **W2 fast lane (v8 P2):** unit-level readiness instead of the wave barrier (`derive-ready-units.sh` after every implementer return), xs fix-round budget 1 → quarantine, xs implementer routed `auto` (→ sonnet, measured cell); and `validate-plan-coverage.sh` must PASS before bolts (`validate-preflight.sh --predictive` refuses the execute-bolts hop on a missing/FAIL coverage state under this lane — a script rail, not prose). Durable form: `lane: lite` in `.mega-sdd/config.yaml` → state engine `derived.lane` → `--resume` and every hop know the lane without the flag. Default v7 chain is unchanged without it.
