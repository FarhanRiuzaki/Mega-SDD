...
- `--vault-layout[=<vault-dir>]` — the layout-2 rung: legacy 7-file vault → 4-file layout-2 (dry-run default, `--apply` executes).
- `--vault-layout=3 [--vault=<vault-dir>]` — **the layout-3 rung: 4-file layout-2 vault → layout-3 `context.md`** (the plan-born shape every consumer reads via ONE resolver). Dry-run by default, `--apply` executes, idempotent (a layout-3 vault is a no-op), dirty-tree refusal. What it does: `vault.md` frontmatter → `context.md` frontmatter (`vault_layout: 3`), sections land under the layout-3 H2 contract (Overview · Architecture · Flows · Data model · Constraints · Open Questions · Decisions — a folded doc's own H2 grouping headings become bold labels so the parsers' H2 slice is not broken); the four docs + `binding.md` / `binding.json` / `claims-ledger.json` are ARCHIVED verbatim under `<vault>/_meta/archive/layout2/` (git mv — nothing deleted; the 00-index residue sections Glossary / Source documents / Auto-Classification / Changelog live only there); `binding.md` is SPLIT per unit by `binding_refs` into `bolts/U-XXX/binding-migrated.json` (claims + CONFLICT blocks with their human RESOLUTION + OQ refs — the human-decision surface is preserved); unit `vault_source` doc names are rewritten to `context.md` (anchors untouched); `derive-vault-json.sh` regenerates `vault.json`. **It never writes `bolts/U-XXX/binding.json`** — that evidence has ONE writer; the script ends by printing the MANDATORY next step: **full JIT re-bind required** (`scripts/rebind-units.sh --cwd=<root> --vault=<vault> --units=all`, or let `execute-bolts --all --lite` JIT-bind each unit at dispatch). A legacy 7-file vault is refused with a pointer to the layout-2 rung (run that first). Vetted core: `plugins/mega-sdd/scripts/migrate-vault-layout3.sh`; pin `tests/migrate-paths/test-vault-layout3-migration.sh`.
