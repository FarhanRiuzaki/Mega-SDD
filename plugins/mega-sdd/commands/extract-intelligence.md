...
- Flags: `--out` (OUTPUT_ROOT / parent dir; default `.mega-sdd/` per `plugins/mega-sdd/references/paths.md` — the KB lands at `<out>/knowledge-base/`), `--seed` (forensic dump, optional), `--max-parallel` (default 5, soft-warn >5, hard cap 8), `--auto`.
