...
Makes `extract-intelligence` reason deeper and catch the cases a write-side-only read misses — *automatically*, every run. Distilled from a deep audit against a real legacy trade-finance codebase (22 findings) and reframed per the user directive: **KB captures business intent + flow; rebuild owns implementation cleanliness — status-naming drift between legacy and rebuild is NOT a gap.**

- **P1–P4 deep disciplines wired into the wave SUBAGENT dispatch prompt** (the block every Wave 1–5 subagent receives, so the deeper reasoning fires automatically — not SKILL.md-only prose the subagents never read): **P1** state writer↔reader provenance + `INSERT…SELECT` clone-inheritance tracing (coupling captured as a business *outcome*, never a pinned legacy value); **P2** enumerate ALL rule/flow sites + entry-point dispatchers (distinct operating models stay distinguishable); **P3** behaviour-as-EXECUTED (debug-code-as-feature, rollback policy, test flags, silent-success); **P4** structural file classification. REPORT BACK gains provenance self-checks; Wave-3 adds a non-blocking `provenance_read_side_thin` advisory.
- **§3a staged-input schema enrichment (reuse-compliant — NOT a parallel artifact):** `input_fields` accepts bare strings (back-compat) OR objects with per-field `mutability`/`visibility`/`conditional`; new optional per-stage deltas (`new_fields_vs_prior` / `hidden_fields_vs_prior` / `promoted_to_mutable_vs_prior`) + `dynamic_disclosures`. Captures the "fields A,B,C at maker; D,E,F at the next stage" case in depth.
- **Extraction Completeness Contract + real validator (advisory):** Wave 5 emits `.extraction-scorecard.json` scoring the five principles; new `scripts/validate-extraction-scorecard.sh` SKIPs when absent, PASSes when consistent, and **FAILs on a hidden gap** (a PARTIAL/MISSING principle with zero `[OPEN]` markers — the silent-drift failure mode). `bind-codebase` consults it as a preflight advisory. B1 hard-block + B2/B3/E1/E2/E3 + downstream generate-units/execute-bolts gates are explicitly scoped Fork-B-future (no prose pretending to HALT without a backing validator).
- Fixture: `tests/fixtures/iter80-extract-deepening/` (Fork-A proof: dispatch wiring + validator verdicts + §3a enrichment). No regressions — iter77 (16/16) + 19/19 code-delivery fixtures green. Spec: `docs/superpowers/specs/2026-06-02-extract-intelligence-deepening-design.md`.

### v3.71.0 — Semantic-depth: staged-input walking skeleton (workflow *meaning* survives the handoff)

Fixes a semantic-depth regression caught from real legacy code: a multi-step workflow (wizard / maker→checker / multi-page form) **stages** its inputs (fields A,B,C at step 1; D,E,F at step 2), but the KB→vault→units→bolts handoff **flattened** it into one "Inputs: A,B,C,D,E,F" list — so the bolt built ONE form where the legacy had a multi-step wizard. Staging was structured *nowhere* and the handoff schema never required it. Walking-skeleton scope: the **staged-input** dimension only (conditional / role-matrix / transition-guard dimensions follow). Consumer-audited first (a 5-lane read-only trace) — `stages:` is an OPTIONAL field that breaks no existing consumer; execute-bolts multi-step is vertical decomposition (deferred Fork-B-future). Advisor-sharpened: a **deterministic `_kb_source` back-reference** (the OQ-ID-class propagation the codebase already uses) instead of fragile fuzzy title-matching.

Five tracks, each commit fixture-proven:
- **Track 1 — schema/contracts:** `knowledge-base-schema.md` `## 3a. Staged inputs` + the `stages:` block (REQUIRED-when-multi-step, conditional → backward-compatible; per-stage `_source` anti-halu anchor); `templates/04-flows.md` `**Stages**` + Mermaid `stateDiagram` + `_kb_source`; `vault-contract.md §stages-propagation`; optional `metrics.flows_with_stages` (type-checked-when-present, never required-on-absence).
- **Track 2 — skill bodies (paired with validators, no prose-only):** extract-intelligence staged-input detection guidance; generate-intent preserve-verbatim-never-flatten.
- **Track 3 — enforcement:** `validate-kb-flows.sh` ADVISORY `kb_flow_staging_missing` (separate `advisories[]` channel — never flips status); new `validate-vault-flow-staging.sh` (PreToolUse **Branch 14**, BLOCKING `vault_flow_staging_drop` via `_kb_source`; SKIPs when no KB / no back-ref / KB had no stages — pre-staging vaults never trip it) + a non-blocking advisory arm `vault_flow_staging_missing` for the dominant wholesale-flatten / PRD-only case (workflow-signal flow with neither stages nor `_kb_source`, which the blocking arm can't see). **Honest coverage:** advisories (KB + vault) are broad; the block is the narrow precise case.
- **Track 4 — remediation:** new `/mega-sdd:enrich-semantics` (`scripts/enrich-workflows-staging.sh`) — two-phase (propose → `--apply`) retro-fit of staging onto an existing KB without re-extraction; re-reads cited legacy `_source`, detects the wizard pattern, allocates fields per stage; consumes the `kb_flow_staging_missing` advisory.
- **Track 5 — proof:** `tests/fixtures/iter77-semantic-depth/` — **21 Fork-A** assertions (incl. the advisory-arm + the non-negotiable **hook-fire gate**) (PreToolUse Branch 14 emits `continue:false` on a real drop, falls through on preserved). Fork-B (LLM skill-body authoring/preservation) explicitly documented as NOT script-asserted. 19/19 code-delivery fixtures still pass.
...
## Anti-hallucination defense (18 layers)
...
18. **Semantic-depth fidelity gates** — workflow *meaning* survives the KB→vault handoff, not just structure. A multi-step workflow's `## 3a` `stages:` block (field→stage allocation, role-per-stage, transitions) must reach `04-flows.md` or `execute-bolts` is blocked: `validate-vault-flow-staging` (Branch 14) follows each flow's deterministic `_kb_source` back-reference → BLOCKING `vault_flow_staging_drop` on a dropped block (the narrow back-ref-preserved-but-dropped case); broad non-blocking advisories cover the rest — `kb_flow_staging_missing` at the KB source + `vault_flow_staging_missing` for the dominant wholesale-flatten / PRD-only case; `/mega-sdd:enrich-semantics` retro-fits staging on an existing KB. Walking-skeleton: staged-input dimension first (v3.71; see What's new)
