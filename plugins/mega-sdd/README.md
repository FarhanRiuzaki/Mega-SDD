...
- **Track 3 — enforcement:** `validate-kb-flows.sh` ADVISORY `kb_flow_staging_missing` (separate `advisories[]` channel — never flips status); new `validate-vault-flow-staging.sh` (PreToolUse **Branch 14**, BLOCKING `vault_flow_staging_drop` via `_kb_source`; SKIPs when no KB / no back-ref / KB had no stages — pre-staging vaults never trip it).
...
- **Track 5 — proof:** `tests/fixtures/iter77-semantic-depth/` — **16/16 Fork-A** assertions incl. the non-negotiable **hook-fire gate** (PreToolUse Branch 14 emits `continue:false` on a real drop, falls through on preserved). Fork-B (LLM skill-body authoring/preservation) explicitly documented as NOT script-asserted. 19/19 code-delivery fixtures still pass.
