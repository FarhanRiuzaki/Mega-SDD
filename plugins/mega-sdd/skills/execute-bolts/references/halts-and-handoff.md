...
The post-flight scan also verifies every modified file has a provenance trailer comment — TWO lines:
...
Per-bolt status is TWO lines (a multi-line per-bolt block would duplicate `_summary.md`, which persists the same detail to disk; ~2.2K tok saved on a 20-bolt batch):
...
<Deferred-OQ re-surface — a pure read of vault.json open_questions[] where
...
handoff:
  ...
  next_action:
    suggested_skill: mega-sdd:detect-drift
    suggested_args: []                     # → ["--scope=<id>"] when the `scope:` block below is present: propagate THIS batch's scope so the chained detect-drift inherits it instead of full-scanning. Stays [] for a single-scope vault.
    rationale: "All bolts executed; recommend a periodic drift check."
