...
The project-wide quality validators that scan GENERATED SOURCE/VIEWS — `validate-cross-cutting-registration.sh`, `validate-ui-quality.sh`, and (for vault edits) `validate-vault-oqs.sh` — fire via PostToolUse on the writer's Write/Edit. PostToolUse DOES fire on bolt-agent writes (it fires inside subagents — AUDIT L1), so the state is not "invisible"; but under `--parallel` / `--per-squad` the project-wide state can lag concurrent write ordering (each validator is a full-glob current-truth re-scan, and the async hook may not have settled). To make the gate state deterministic: after each bolt batch completes, the **main-thread controller** explicitly bash-invokes those validators against `$PROJECT_ROOT` so the gate reflects current truth regardless of write ordering. This is **defense-in-depth** on top of the detect-and-block-next contract — not a load-bearing compensation for an invisible write.
