...
> **Load condition:** load this file ONLY when the invocation is multi-unit — `--all`, `--parallel`, `--per-squad`, `--squad=`, or `--module=`. A single-unit run never needs it: the per-bolt drift check and the B2 full-suite gate (which apply to every invocation) live in `halts-and-handoff.md`.
...
3. Wave dispatch (the default on `--all`, and what `--parallel` selects elsewhere): the **main-thread controller** groups units with no shared dependency and dispatches them **concurrently** — multiple `bolt-implementer` Agent calls in one message — each unit still running the full review panel (`bolt-implementer` → the parallel blind lenses per `review-panel.md`). **Overlap rail (independence = BOTH conditions):** no `depends_on` edge between them AND pairwise-disjoint `target_files` — units whose whitelists intersect are never in the same wave; they serialize (a silent data race on a shared file is worse than slower execution). This is **depth-1** (the controller stays in the main thread; it never forks a sub-controller that would then need to dispatch the bolt agents — that would be depth-2, which the runtime forbids). `subagent-driven-development` is an optional *technique* hint, not a nested dispatch.
   ...
   - **In-flight cap.** Wave dispatch is bounded by an in-flight cap (`config.yaml parallel_max:`, default **4** concurrent `bolt-implementer` agents — the same bound `--per-squad` uses and the value `vault_layouts.parallel_max` reads): on the DEFAULT lane a wave wider than the cap dispatches in cap-sized slices, each slice completing its review tail before the next slice (under lane lite the slice rule is replaced by per-unit pipelining — see the lite clause below). Unbounded fan-out is never dispatched — each in-flight bolt later fans out its own panel lenses, and the platform's concurrent-task comfort zone is finite.
   ...
   - **Same-tree concurrency valves.** Disjoint `target_files` (the overlap rail) removes content races; the shared git INDEX is still a brief contention point — a transient `index.lock` failure during a concurrent commit is retried briefly by the implementer (agent contract), never reported as BLOCKED. **Wave commit rail (spec 2026-08-30 §1.3):** every commit made while a bolt is in flight — the implementers' AND the controller's (evidence, spec amendments, a BLOCKER note) — stages by **explicit pathspec**; the PreToolUse hook DENIES `git add -A/--all/.`, `git commit -a/--amend`, `git stash`, `git reset --hard` while any `<vault>/bolts/U-XXX/dispatch-prompt.md` is newer than its `postflight.json` (the in-flight predicate, `vault_layouts.inflight_units`). Field measurement behind it: one BLOCKER commit swept 1,031 lines of two siblings' half-written tests; `--amend` ×11. **In-run gate (spec §1.1):** each `bolt-implementer` dispatch re-evaluates the execute-bolts gate aggregator with in-run semantics — B2 (full suite) stays a run-boundary gate checked at `--all`/`--sprint` entry; B1/B4/orphan issues of units in flight are pending, not missing, and do not block a sibling's fix round. A unit dispatched WITHOUT a dispatch-prompt (hand-typed) is never in flight and is evaluated in full. Acceptance-test runs inside a wave share project test state (a test DB, fixtures, caches): on a project whose suite is not concurrency-safe, run the batch with `--worktree` (per-bolt isolation) or drop `--parallel` at the chain's Edit step — a false red from shared test state is a `test_fail` halt the wave semantics then honor.
   ...
   - **Lane lite (v8 W2 — `derived.lane: lite` / `--lite`): unit-level readiness replaces the wave barrier.** MEASURED on the clinic baseline (`research/2026-09-11-v8-p2-report.md §1`): 8 sequential waves cost 17–45 min each and a 1-unit tail wave still 21–25 min because the dependent waited for the upstream unit's whole panel, not its evidence. Under lite the controller runs `bash <plugin-root>/scripts/derive-ready-units.sh --cwd=<root> --vault=<vault>` after every implementer return and dispatches every `ready` unit (up to the in-flight cap): a unit is ready when EVERY `depends_on` unit is `implemented` with `acceptance.json` pass AND `postflight.json` pass — evidence written, while that upstream unit's panel / verifier may still be running in the background. The upstream unit's panel findings still gate THAT unit (fix round → xs budget 1 → quarantine); the dependent's own panel + verifier review the integration. A dependency that is quarantined, red, stale, or missing evidence BLOCKS its dependents (never pipeline against missing/red evidence). Every per-unit gate at dispatch (in-run recompute, B1–B4, whitelist, CONFLICT) is unchanged — only the ORDER of review vs dependent dispatch moves. **P3 pipelining rules (`research/2026-09-15-v8-p3-report.md §2` — MEASURED on the clinic lite arm: the controller still ran cap-sized slices with a full barrier, 45 % of the net bolt-stage had ZERO implementers running, mean in-flight 1.46 of cap 4, because the panel-evidence gate read a sibling's not-yet-merged ledger as missing; the DAG was never the limiter — 7 root units, 3 of them idle for 39–221 min):** (1) the slice sentence above does NOT apply under lite — after EVERY implementer return run THAT unit's detect-after (L0 → postflight → acceptance), dispatch ITS panel lenses immediately (one message, blind — never batch the panels of a slice, never wait for the slowest sibling), then run `derive-ready-units.sh` and top up dispatch to the cap with every `ready` unit; **L1 (spec `2026-09-16-clinic-levers-design.md`, MEASUREMENT PENDING): the top-up is the script's `dispatch_plan.dispatch_now` list VERBATIM — one Agent call per listed unit, in that order, in the SAME message; the controller never composes a burst, never waits for a second return to "batch", and `slots: 0` means wait for the next return (never a timer). The plan already subtracts the in-flight set (`vault_layouts.inflight_units`, the definition the gate and the wave rail read) from `parallel_max` and orders ready units critical-path-first (direct-dependent count desc);** (2) a unit whose postflight + acceptance are PASS and whose L0 record is written but whose ledger is still merging is **panel-pending, not evidence-missing**: the in-run gate (`vault_layouts.panel_pending_units`) drops its `panel_evidence_missing` issue for the next `bolt-implementer` dispatch, bounded by `parallel_max` panel-pending units at once (more than the cap = the controller is outrunning its review obligation → the full panel-evidence verdict applies again); `l0_evidence_missing` is never dropped; the run boundary (Skill entry, Stop hook) still enforces every ledger; (3) evidence commit + `bolt_completed` per unit as soon as ITS ledger merges clean — never per slice.
