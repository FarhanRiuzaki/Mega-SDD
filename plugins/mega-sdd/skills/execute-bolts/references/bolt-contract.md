...
WITHOUT it is listed as `bypass_commits[]` in `_batch-suite.json` (written by `run-full-suite.sh --base=<sha>`, mirrored in `_summary.md` — `halts-and-handoff.md §B2`). The `SDD-Acceptance: v5`
