...
- **Provenance values block** (per-unit: unit_id, vault sha256, claim ids + texts, anchors, active Hard rules — the agent fills `version` + `unit_id` + `vault sha256` into its TWO-line trailer (8.0.2) and copies the claim ids into its bolt-report `claims:`; the anchors and Hard-rule texts stay informational in the block).
