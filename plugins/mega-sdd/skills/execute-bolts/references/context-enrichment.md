...
**Amendment history:** the measurement/amendment corpus (withdrawn boxes, struck derivations, dual byte-accounting tables, fix narratives, removed known-open rows) is archived VERBATIM in `docs/superpowers/specs/2026-08-11-context-enrichment-amendment-archive.md` — **"the archive spec"** wherever this file says "amendment history → the archive spec". Nothing in the archive is operative; the operative sentences those blocks carried were extracted and remain below.
...
- **Provenance values block** (per-unit: unit_id, vault sha256, claim ids + texts, anchors, active Hard rules — the agent fills `version` + `unit_id` + `provenance_path` into its TWO-line trailer and copies the claim ids into its bolt-report `claims:`; the vault sha and the anchors (with STALE flags — an implementer signal) stay in the block; `hard_rules_active` is a COUNT — the verbatim text already sits in the same file's `## Hard rules`).
  - **`claims:` MUST carry the claim ID `C-NNN`, not a confidence label.** Shape: `- C-NNN "<claim text>"`, one line per implemented claim. The agent's bolt-report `claims:` line is `- C-NNN "<claim text>"` and this block is its ONLY sanctioned source (`agents/bolt-implementer.md §Provenance trailer` + §Report; the code trailer itself does not carry claims), so a block that emits `- HIGH "notes"` leaves the agent with two options — omit the id, or back-derive one from `binding_refs`. The second is the fabrication this block exists to prevent, and post-flight only checks that a trailer is PRESENT, so a malformed-but-present trailer passes every gate. The id is available where the claim is resolved; carry it through. The confidence label belongs in the T2 `confidence_labels` section, which is where it already is.
  - `hard_rules_active` is a COUNT of the mechanical rules the shared B1 lexer extracted — the verbatim text is already in this file's `## Hard rules`, and no trailer line copies it; still no synthesized ids (ids are never minted — see §Re-decided amendments).
...
- Anti-context block (DO NOT MODIFY / DO NOT REPLICATE / DO NOT WRITE / DO NOT COMMIT IF). Each line names its own source; each is omitted whole when its source yields nothing.
  - **`DO NOT MODIFY:` is a LABELLED UNION of two sources, not one substituted for the other.** (a) the `[LOCKED]` entries of the KB's `data-mutation-policy.md` when that file exists — a real artifact: `extract-intelligence` synthesis writes it at the KB root when ≥1 `[LOCKED]` claim exists (the legacy numbered tree kept it under `99-rebuild-architecture/`; the builder probes both) and `generate-intent` reads it (the DO-NOT-MODIFY union below is what enforces it at dispatch); and (b) the unit's own `## Hard rules` lines matching `^(DO NOT|MUST NOT|NEVER)\s+modify\s+(.+)`. **Every entry carries the source it came from** (`(source: data-mutation-policy.md §<section>)` / ``(source: U-XXX.md `## Hard rules`)``). Either source absent contributes nothing and is recorded in `sections_omitted`; both absent omits the line. The failure this rule exists to prevent: a legacy-rebuild project whose KB marks `LegacyLedger` `[LOCKED]` in `data-mutation-policy.md`, on a unit whose generated Hard rules do not restate it, ships a bolt that modifies the locked file. Substituting a different source under the original's label is the subtlest form of invariant-#5 breach — the citation is real, the assertion is not.
  - **The policy-table parser is STRUCTURAL, in both directions.** The builder locates `data-mutation-policy.md` under the KB roots, reads both `## Per-locked-field policy` and `## Entity-level summary` with per-table marker semantics, and stamps a per-entry source. The header row is dropped **positionally** — it is row 1, and it is a header because row 2 is the `|---|` alignment row — never by a header-word allowlist, so a table headed `| Field | Tier | Policy |` does not emit `Field` as a locked path. The converse holds too: a dash-only FILLER row never eats the `[LOCKED]` entry above it (the alignment row is row 2 and nothing else), and any row the parser discards is recorded in `sections_omitted` with a reason. Both directions are rails — inventing a locked path and silently losing one are the same invariant-#5 class. Pinned by `tests/derived-artifacts/test-dispatch-prompt-builder-shape.sh` §W (W1–W9).
...
- **Reuse index path (whenever `.mega-sdd/codebase/reuse-index.yaml` EXISTS — even when `reuse_candidates` is empty; never for an absent file: the absence is recorded as `t1.reuse_index_line`).** ONE canonical text — **the template's wording WINS** (`bolt-dispatch-prompt.md §Reuse index` is the emitted artifact's own shape, and it names Iron Rule 4, which is the agent-side contract the line exists to trigger). Verbatim, three lines:
  ```
  ...
  ```
- **`unit.reuse_candidates`** (fast-path hint — include when present; empty list is fine to omit the hint line; the reuse-index path line above ships iff the index file exists).

## T2 budget tracker

Running T2 consumption tracking + progressive section-level truncation by priority (never a single halt at cap).

```
running_budget = {
  cap_hard:      12288   # halt-conjunction term (b). See the AMENDMENT below.
  cap_target:    9216    # 9KB total target (advisory)
  cap_t1:        12288   # T1 REPORTING THRESHOLD (see the AMENDMENT below)
...
**Scope of this amendment: the NUMBERS in `running_budget` and nothing else.** The 9-priority truncation table with every cascade rung and drop floor, the `starterkit-enrichment.md` 7-rung slice ladder, and the three-way `dispatch_prompt_too_large` conjunction in §Halt path are UNCHANGED by it. The numbers were re-derived from measurement, not guessed: **n = 123 builder runs** (41 distinct units × 3 context arms, byte decomposition verified byte-exact on every run). The full corpus — the T1 distribution and non-body-T1 floor, the byte decompositions, the amended-numbers table, the WITHDRAWN algebraic derivation and the struck `cap_hard ≤ 12 906` ceiling, the empirical halt-firing table with its non-vacuity control, and the measured T1-recovery figures — is archived verbatim: amendment history → the archive spec.

What stands, operatively:

- **The current constants are the ones in `running_budget`** (§T2 budget tracker): `cap_t1 = 12288`, `cap_t2 = 10240`, `cap_hard = 12288`, `cap_target = 9216`. Change them HERE first, then the builder (this file's header rule).
- **`cap_t1` is a REPORTING THRESHOLD, not a budget.** Nothing branches on it: it feeds one warning and one report echo. T1 is never truncated and the unit body is embedded verbatim, so *no* value of `cap_t1` bounds T1. Crossing it is a **`generate-units` atomicity smell** (a unit too big to be one PR-sized bolt), not a budget complaint.
- **`cap_t1 + cap_t2 == cap_hard` is EXPLICITLY RETIRED.** That identity (2048 + 10240 = 12288) held by arithmetic coincidence, not by design; `cap_t1` and `cap_hard` are different instruments — a reporting threshold and a halt term — and are not required to relate. **Do not re-derive one from the other.**
- **The three-way conjunction in §Halt path is REACHABLE — proven EMPIRICALLY, never by algebra.** Four distinct units plus a lean probe reached `exit 1` with a populated `halt` object, with a working non-vacuity control. An algebraic derivation ("(a) ⟹ (b)") is false — term (a) is satisfiable vacuously over an empty section set — so nothing may be built on it. All three terms discriminate; none is derivable from another (§Halt path).
- **`cap_hard = 12288` stays, and raising it is measurably not the lever:** at `cap_hard = 22528` the halt would never have fired on any unit in the corpus, and a full second builder copy at 13312 halted 28/28 identically to 12288. **No ceiling for a future raise is published** — the struck `12 906` figure rested on the withdrawn derivation; re-establish a bound from measurement before proposing any raise. A moat test still pins `12906` as a regression rail — see §Known open.
- **T1 is unbounded in principle** (never truncated, unit body verbatim), and the measured maximum was CONSTRUCTED at `unit-schema.md`'s documented upper end — real fixtures top out lower. Do not lower `cap_t1` from a re-measurement on real production units alone.
- **Measured T1 recoveries are BACKLOG** (see §Named backlog); if all three ship in one tranche, `cap_t1` becomes 10240 — `cap_hard` stays 12288 either way, since shrinking T1 only *increases* term (b)'s reachability margin.
...
| 2 | `historical_memory` | (REMOVED — always `sections_omitted`; priority slot retained for order stability) | — |
...
| 7a | `framework_pack_rules` | top 5 → top 3 → top 1 | keep top 1 always |
| 7b | `code_style_slice` | all bullets → first two → first bullet (the FIRST `## Code style (self-documenting)` section in the chain — most-specific pack wins, no merge; spec `2026-09-16-code-style-playbook-design.md §3`) | first bullet (doc-comment tool + read by) — never dropped |
...
**Tier enumeration (contract completeness):** tier 8 carries THREE sections and is enumerated 8a/8b/8c in its pinned order; tier 3 is likewise enumerated 3a/3b — `symbol_slice` joins at the same priority as `reuse_slice`, stepped after it (3a first) one rung per pass, same tie discipline as tier 8; tier 7 is enumerated 7a/7b the same way — `code_style_slice` steps after `framework_pack_rules`. The nine PRIORITIES themselves are unchanged. **`map_patterns` is kept, not removed** — it is the only pattern source a regex-tier scan produces (see §Map §6 fallback), so deleting it would drop real content on exactly the projects with the least context.
...
- **`reuse_slice`, `symbol_slice`, `depends_on_summaries`, `framework_pack_rules`, `code_style_slice` and `design_slice` never reach `""`** — their drop floor is a real surviving payload (`reuse_slice`'s floor is the literal `+N more — read reuse-index.yaml directly` line; `design_slice`'s is system+style only). `validation_hints` and `confidence_labels` do drop to empty; `historical_memory` is always omitted. (Sections are named here rather than numbered — a numbered list silently re-points when a tier is enumerated.)
...
- Code style slice (7b): the `- ` bullets of the FIRST `## Code style (self-documenting)` section in the resolved chain (most-specific pack wins, NO merge — one owner per delta; `_universal.md` never carries one), rendered under `## Code style (from <pack>.md §Code style — stack delta over Iron Rule 6; a style rule, not a gate)`. It is the stack's DELTA over the agent-carried comment rule (`bolt-implementer.md` Iron Rule 6): the doc-comment tool + who READS it, the skip/write vocabulary, idiomatic names. Absent → omitted with the chain named; unresolved chain → the UNKNOWN-not-empty wording. Nothing in it is validated post-flight (F.5).
- **Constitution clauses — the selector is a narrowed three-way intersection.** The phrasing "ONLY clauses referenced in this unit's `vault_source` sections" is genuinely **unimplementable and must not be restored**: `vault_source` is a scalar and nothing keys a clause to a vault section. A bare `\b[A-F]-\d{3}\b` scan of the whole unit file is too wide, and its cost lands on the one section that can force the halt: a clause pulled by a token in a code sample enters priority 9, which is NEVER truncated, so a false positive can push a dispatch over the cap that would otherwise ship. The selector is the intersection of three real inputs, in this order:
  ...
  **Emitted heading.** The section heading is `## Constitution clauses (cited in this unit, resolved in the constitution §C)`. A heading is a claim about provenance; it gets the same discipline as any other — it must state the selector that actually ran, never a source it did not use.
...
- Historical memory: REMOVED (the builder records the priority-2 slot in `sections_omitted` and emits nothing).
...
- **Confidence labels per claim — ONE taxonomy.** The enum is exactly **`HIGH | MEDIUM | LOW | OQ`** and it is the same enum in this file and in `bolt-dispatch-prompt.md`; neither file may carry a different one.
...
**Express-born projects (no scan ever run) have NEITHER leg** — no starterkit-context.yaml AND no codebase-map §6; the builder's `omit("map_patterns", …)` records the honest omission and the bolt proceeds on pack conventions + the symbol/reuse slices. Running `scan-codebase` on demand restores both legs.
...
> Why this slice exists (the clinic-project audit gap): generate-intent wrote a full
...
> INSIDE the starterkit branch (`starterkit-enrichment.md`) — greenfield projects have no
...
ELSE build design_slice:
  ...
  ELSE:
    ...
    # (Emitting the literal pseudocode `design_slice.note = "..."` into the
    #  agent-facing prompt makes the one line that tells the subagent to raise
  ...
  design_slice.baseline   = design-intelligence/modern-baseline.md
                            ...
                            # must aim for the ceiling, not stop at the floor.
```
...
implementer and the reviewer share one contract. **The slice reaches the lens as a PATH to a controller-written lens-input file, not as a pasted string.** The builder writes the emitted section verbatim to
...
**Why the PATH carrier — a measured COST fix, not an abandoned rail.** Returning the slice as `design_slice_text` on stdout and pasting it verbatim into the lens prompt bills the same string twice per greenfield UI bolt — and the outbound leg alone is the larger term. The rail is intact and is stated below; only the carrier differs. Exact measured figures, method, spread and per-channel breakdown: amendment history → the archive spec, and `docs/superpowers/specs/2026-07-30-token-and-latency-optimization.md` §2b.

**The rail, stated at its PURPOSE:**

> **No lens may receive a path that reaches another lens's verdict or the implementer's self-report.**

That is what blind review protects: a lens forming its opinion before it can see anyone else's. It is NOT a blanket prohibition on paths — an absolute rail that the shipping procedure (`review-panel.md §Live-app capture`) violates is not a rail; it is an ambiguity a reader resolves by guessing.

Applying the purpose-scoped rail:

- **`<vault>/bolts/U-XXX/dispatch-prompt.md` — still forbidden to every lens.** That directory is where `bolt-report.md` lands, and on a `--resume`, a retry, or any re-run over a previously-bolted unit it already holds the prior attempt's `## Review panel` verdicts and `bolt_self_report`. A lens with Read/Grep is one `Glob` of the parent directory away from all of it. The prohibition on the bolt dir stands.
...
> **Reading the bullets above.** The first bullet's own qualifier — *"should only fire when `constitution_clauses` alone exceeds budget (progressive truncation absorbs most cases first)"* — **is terms (a) and (c) of §Halt path stated in prose.** The bullet was never an independent `total > cap_hard` trigger; it is §Halt path's conjunction restated loosely. **§Halt path is the operative definition, in one place, unchanged.** The qualifier is load-bearing: read WITHOUT it, `total > cap_hard` alone fires on a quarter of measured healthy dispatches — including runs with `truncations: []` and nothing wrong with them — a tax, not a gate.
>
> **DISCLOSURE — `total` is the T1+T2 ACCOUNTING, not the size of the file on disk.** Four blocks are written into `dispatch-prompt.md` OUTSIDE `total = consumed_t1 + consumed_t2`: (1) the `PROVENANCE — omissions` appendix, (2) the `TIER 2` banner, (3) the `### T2 budget tracker` block itself, (4) the `TIER 3` banner + pointer list. **The title banner and the `TIER 1` banner are NOT among them** — both are appended into `t1`, so they are inside `consumed_t1` already. None of the four sits on any rung of the 9-priority cascade, so under budget pressure the ladder shrinks contract content while these are untouchable — and the tracker injected INTO the prompt reports a `total:` for a file that is larger than that, while `agents/bolt-implementer.md` Rule 0 tells the implementer to read that file in full. **This note discloses the gap; it does not close it** — closing it is a builder change, carried in §Known open. The measured magnitude by arm, the per-run byte identity, and the two block-boundary accountings (which must never be mixed): amendment history → the archive spec.
...
| `framework_pack_rules` (P7a), `code_style_slice` (P7b), `design_slice` / `starterkit_slice` (P8) | **held at their own drop-floor rung** — never fully dropped; the design lens reads the post-cascade text, so implementer and reviewer keep ONE (floor-level) contract |
...
**Why exit 4 exists.** Without it, *any* unhandled exception in the builder — an unimportable shared lib, a malformed unit, the `assert len(rules) == len(levels) - 1` in the section ladder — would also exit 1, with zero stdout and no file written. A controller following the exit-1 contract would read that as `dispatch_prompt_too_large`, look for a `halt` object that does not exist, and find the bolt directory still holding **the PREVIOUS run's `dispatch-prompt.md`** — which the exit-1 contract explicitly tells it to trust as forensic evidence. That is a crashed build dispatching the previous unit's prompt to a subagent. Two requirements close it, and BOTH are mandatory:
...
**Sizing:** this object is deliberately slim — `sections_omitted` forensics the controller never reads would be re-billed as resident context on every subsequent controller turn. The measured per-arm sizes, the exact-path assumption and the path-length law: amendment history → the archive spec. Never average the arms, and any quoted figure must state which project-root length (the path appears 2× on a non-UI unit, 3× on a UI unit) it assumed.

**Required:** by default stdout carries only the controller-consumed keys plus the small accounting the report line needs — `status`, `halt`, `inline_core`, `prompt_path`, `total_bytes`, `t1_bytes`, `t2_bytes`, `truncations[]`, `warnings[]`, `soft_halts[]`, and `design_slice_path` (below). **`sections_omitted[]` is NOT on default stdout.** It is forensics and it has a better home: it is already written into the prompt file's own provenance section, where the auditor and `validate-dispatch-prompt.sh` can both see it.
...
| `--quiet` | **nothing** | **FORBIDDEN for the controller.** It suppresses the JSON entirely and therefore `inline_core` and `design_slice_path`, which are the only reason the controller runs the builder. |
...
**The rubric travels as a controller-written lens-input FILE; a `design_slice_text` key — a string on stdout the controller pastes verbatim into the lens prompt — is FORBIDDEN** (§Handing the slice to the design lens carries the purpose-scoped rail that makes a path legal here; the measured double-billing: amendment history → the archive spec). The rubric is not weakened: it is the same bytes, written once, read once.
...
**Cost of this field — MEASURED, its own line item, never folded into a ratio.** The per-channel table (input stdout member vs output raw path — two different units on purpose): amendment history → the archive spec.

> **The write is gated on `ui_bearing` — the SAME gate the `design_slice` section (priority 8c) uses.** `starterkit_relevance` carrying `ui_ux` is NOT sufficient: a non-`ui_bearing` unit emits no `design_slice_path`, writes no lens-input file, and records the reason in `sections_omitted` — that is what keeps *"omitted entirely when the unit is not UI-bearing"* above true, and `review-panel.md §Tier selection`'s *"pure-backend units never pay for it"* true with it. **The unit's own starterkit slice still ships INTO THE PROMPT** when it declares `ui_ux` relevance — the gate withholds a lens input, it does not delete content. Pinned by `tests/derived-artifacts/test-dispatch-prompt-builder-shape.sh` §T4.

**Why the file and not the paste:** a paste is billed on BOTH channels for the same bytes — inbound as a tool result, outbound as the controller's re-emission into the lens prompt (measured: amendment history → the archive spec). **This is not a retreat from the blind-review rail:** the rail is *"no lens may receive a path that reaches another lens's verdict or the implementer's self-report"*, `<vault>/lens-inputs/` holds neither by construction, and the prohibition on `<vault>/bolts/U-XXX/dispatch-prompt.md` and its directory is unchanged.
...
**No validator call here.** `validate-dispatch-prompt.sh` is ADVISORY and re-runs under the analyze skill (`run-analyze.sh` V16); no hook fires it per bolt. **The controller has no wiring step here and MUST NOT be given one** — a prose obligation that duplicates a dispatcher rots (`plugins/mega-sdd/CLAUDE.md` — *gates > rules > hooks*).
...
Five spec passages were re-decided on their own merits; the verdicts and the grounds are recorded here so the next reader can audit the decision rather than discover it.
...
- **Do NOT file (or re-file) "restate the halt without the `total > cap_hard` term".** Term (b) is not redundant — under a vacuous term (a) it is the ONLY size discriminator, and executing this would make the halt fire on any unit citing one constitution clause regardless of size — a chain-killer.
...
- **`## Prior failure context`** — CLOSED as REMOVED: there is no memory lane for it to read; the retry path's failure context rides the finding ledger (`findings.json`, pointer re-dispatch) instead — pipeline evidence, not memory.

## Known open (carried deliberately — named so they are not lost)

**Every row below was re-verified against the shipped code, one at a time.** A known-open list that names fixed defects is worse than none, because it teaches a maintainer to distrust the whole table — so rows the code no longer matches are removed, with the removal recorded (amendment history → the archive spec). Each surviving row is a real defect or a real cost; naming it is the requirement, fixing it is a separate change with its own tests.

| # | what | why it is not fixed here |
|---|---|---|
| 1 | **The pack-resolver call sites outside the builder still swallow non-zero exits.** **11 call sites across 7 validators** — `validate-flow-coverage.sh` (4), `validate-sibling-consistency.sh` (2, plus its `--fanout-parity` and `--cross-cutting` modes), `validate-unit-spec.sh`, `validate-ui-quality.sh` (+ `--deferral`) — every one of the form `X=$(bash "$PACK_RESOLVER" … ) \|\| X=""`. On the Windows target the App-Execution-Alias stub makes "resolver failed" indistinguishable from "packless project", so those gates go dark at exit 0. The discrimination contract is in §Pack-resolver exit codes and is honoured by the builder and `validate-dispatch-prompt.sh` only. | Pre-existing, spans seven scripts including PreToolUse-blocking gates; deserves its own change and its own portability tests. |
| 3 | **A moat test still pins `cap_hard ≤ 12906`** as a regression rail (`tests/moat/test-dispatch-prompt-cascade.sh`, assertion I4). That ceiling was derived from the withdrawn arithmetic and is struck from this file. | Weakening or deleting an assertion without a code change that justifies it is out of scope here. The pin must be **re-grounded by measurement** or dropped in a change that says so; until then it is documented here so nobody reads `12906` as a live bound. |
| 4 | **`total` under-reports the emitted file.** Four blocks — the `PROVENANCE` appendix, the `TIER 2` banner, the `### T2 budget tracker` block and the `TIER 3` banner + pointer list — are written into `dispatch-prompt.md` outside `consumed_t1 + consumed_t2`, and none sits on a cascade rung. (The title and `TIER 1` banners are NOT among them; they are inside `consumed_t1`.) §Size check DISCLOSES this and the tracker now reports `file_total` beside `total`; neither closes it. | Closing it is a builder change (budget the appendix into the cascade, or make the on-disk size the budgeted figure). The enumeration SAYS the right thing; what the cascade COUNTS is unchanged. |
...
| 6 | **A failed run can now leave a STALE prompt for the advisory validator to pass.** This is the second-order consequence of removing the pre-assembly unlink, and removing it was still correct (the unlink destroyed correct artifacts). `validate-dispatch-prompt.sh` (run under analyze, V16) globs `<vault>/*/bolts/*/dispatch-prompt.md` with **no freshness check of any kind** (verified: no mtime read, no exit-code input). So after an exit 2 or exit 4 it can read the PREVIOUS attempt's prompt and write a passing advisory state where an unlink would have made it go dark on "no emitted prompt found". | The fix is a freshness signal the validator can act on (the builder's exit code is known to the controller that runs it), not a return of the unlink — reintroducing the unlink would trade a stale advisory state for a destroyed artifact, which is the worse of the two. It is a validator + hook change, and it is advisory-surface only: no dispatch decision is made from this state. |

*(Rows whose complaint the shipped code no longer matched were removed rather than carried — the removed list, verbatim: amendment history → the archive spec.)*
