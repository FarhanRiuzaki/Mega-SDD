...
- **Provenance values block** (per-unit: unit_id, vault sha256, claim ids + texts, anchors, active Hard rules — the agent fills `version` + `unit_id` + `provenance_path` into its TWO-line trailer (8.0.3) and copies the claim ids into its bolt-report `claims:`; the vault sha and the anchors (with STALE flags — an implementer signal) stay in the block; `hard_rules_active` is a COUNT since 8.0.3 — the verbatim text already sits in the same file's `## Hard rules`).
  - **`claims:` MUST carry the claim ID `C-NNN`, not a confidence label.** Shape: `- C-NNN "<claim text>"`, one line per implemented claim. The agent's bolt-report `claims:` line is `- C-NNN "<claim text>"` and this block is its ONLY sanctioned source (`agents/bolt-implementer.md §Provenance trailer` + §Report; the code trailer itself no longer carries claims — 8.0.2), so a block that emits `- HIGH "notes"` leaves the agent with two options — omit the id, or back-derive one from `binding_refs`. The second is the fabrication this block exists to prevent, and post-flight only checks that a trailer is PRESENT, so a malformed-but-present trailer passes every gate. The id is available where the claim is resolved; carry it through. The confidence label belongs in the T2 `confidence_labels` section, which is where it already is.
  - `hard_rules_active` is a COUNT of the mechanical rules the shared B1 lexer extracted (8.0.3) — the verbatim text is already in this file's `## Hard rules`, and the trailer line that used to copy it is gone; still no synthesized ids (the 2026-07-31 re-decision was about ids vs text, and ids are still never minted — see §Re-decided amendments).
...
- **The current constants are the ones in `running_budget`** (§T2 budget tracker): `cap_t1 = 12288` (was 2048), `cap_t2 = 10240`, `cap_hard = 12288` (UNCHANGED), `cap_target = 9216`. Change them HERE first, then the builder (this file's header rule).
...
- **`cap_t1 + cap_t2 == cap_hard` is EXPLICITLY RETIRED.** That identity (2048 + 10240 = 12288) held by arithmetic coincidence, not by design; `cap_t1` and `cap_hard` are different instruments — a reporting threshold and a halt term — and are no longer required to relate. **Do not re-derive one from the other.**
- **The three-way conjunction in §Halt path is REACHABLE — proven EMPIRICALLY, never by algebra.** Four distinct units plus a lean probe reached `exit 1` with a populated `halt` object, with a working non-vacuity control. The previously-published algebraic derivation ("(a) ⟹ (b)") was WITHDRAWN as false — term (a) is satisfiable vacuously over an empty section set — and everything built on it is struck with it. All three terms discriminate; none is derivable from another (§Halt path).
...
- **Measured T1 recoveries are BACKLOG, not this tranche** (see §Named backlog); if all three ship in one tranche, `cap_t1` becomes 10240 — `cap_hard` stays 12288 either way, since shrinking T1 only *increases* term (b)'s reachability margin.
...
**Tier enumeration (contract completeness):** tier 8 carries THREE sections and is enumerated 8a/8b/8c in its pinned order; tier 3 is likewise enumerated 3a/3b — `symbol_slice` joins at the same priority as `reuse_slice`, stepped after it (3a first) one rung per pass, same tie discipline as tier 8; tier 7 is enumerated 7a/7b the same way (8.1.0) — `code_style_slice` steps after `framework_pack_rules`. The nine PRIORITIES themselves are unchanged. **`map_patterns` is kept, not removed** — it is the only pattern source a regex-tier scan produces (see §Map §6 fallback), so deleting it would drop real content on exactly the projects with the least context. (The two dated enumeration amendments and the rowless-section defect they closed: amendment history → the archive spec.)
...
- Code style slice (7b, 8.1.0): the `- ` bullets of the FIRST `## Code style (self-documenting)` section in the resolved chain (most-specific pack wins, NO merge — one owner per delta; `_universal.md` never carries one), rendered under `## Code style (from <pack>.md §Code style — stack delta over Iron Rule 6; a style rule, not a gate)`. It is the stack's DELTA over the agent-carried comment rule (`bolt-implementer.md` Iron Rule 6): the doc-comment tool + who READS it, the skip/write vocabulary, idiomatic names. Absent → omitted with the chain named; unresolved chain → the UNKNOWN-not-empty wording. Nothing in it is validated post-flight (F.5).
- **Constitution clauses — the selector is a narrowed three-way intersection.** The pre-2b phrasing "ONLY clauses referenced in this unit's `vault_source` sections" is genuinely **unimplementable and cannot be restored**: `vault_source` is a scalar and nothing keys a clause to a vault section. A bare `\b[A-F]-\d{3}\b` scan of the whole unit file is too wide, and its cost lands on the one section that can force the halt: a clause pulled by a token in a code sample enters priority 9, which is NEVER truncated, so a false positive can push a dispatch over the cap that would otherwise ship. (The amendment narrative: amendment history → the archive spec.) The selector is the intersection of three real inputs, in this order:
...
**Why the PATH carrier — a measured COST fix, not an abandoned rail.** The retired shape returned the slice as `design_slice_text` on stdout and required the controller to paste it verbatim into the lens prompt, billing the same string twice per greenfield UI bolt — and the outbound leg alone was the larger term. The rail it was protecting is intact and is stated below; only the carrier changed. Exact measured figures, method, spread and per-channel breakdown: amendment history → the archive spec, and `docs/superpowers/specs/2026-07-30-token-and-latency-optimization.md` §2b.
...
That is what blind review protects: a lens forming its opinion before it can see anyone else's. It is NOT a blanket prohibition on paths — an absolute rail that the shipping procedure (`review-panel.md §Live-app capture`) violates is not a rail; it is an ambiguity a reader resolves by guessing. (The falsified round-2 absolute wording: amendment history → the archive spec.)
...
**Sizing:** this object is deliberately slim — the retired shape carried 41–63 % `sections_omitted` forensics the controller never reads, re-billed as resident context on every subsequent controller turn. The pre-fix baseline, the measured per-arm current sizes, the exact-path assumption and the path-length law: amendment history → the archive spec. Never average the arms, and any quoted figure must state which project-root length (the path appears 2× on a non-UI unit, 3× on a UI unit) it assumed.
...
**The rubric travels as a controller-written lens-input FILE; the predecessor key `design_slice_text` — a string on stdout the controller pasted verbatim into the lens prompt — is RETIRED and must not return** (§Handing the slice to the design lens carries the purpose-scoped rail that makes a path legal here; the measured double-billing that forced the change: amendment history → the archive spec). The rubric is not weakened: it is the same bytes, written once, read once.
...
Five spec passages were amended during the wiring pass to match the new builder. Each was re-decided on its own merits; the verdicts and the grounds are recorded here so the next reader can audit the decision rather than discover it.
...
- **Do NOT file (or re-file) "restate the halt without the `total > cap_hard` term".** Term (b) is not redundant — under a vacuous term (a) it is the ONLY size discriminator, and executing this would make the halt fire on any unit citing one constitution clause regardless of size — a chain-killer. (The struck backlog item and the withdrawn derivation it rested on: amendment history → the archive spec.)
...
*(Rows whose complaint the shipped code no longer matched were removed rather than carried — the "Removed in round 4" list, verbatim: amendment history → the archive spec.)*
