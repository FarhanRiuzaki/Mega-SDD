...
2.5. **Positive UI obligations when writing a view (code-delivery slice E — defense-in-depth):** when a `target_files` path is a view file (a path matching the active framework pack `## UI quality signatures` → `view_glob`), the generated view MUST be fit for a human operator, NOT a raw scaffold dump. Concretely, satisfy the pack signatures: (a) NO `scaffold_tells` — give the page a human title (never the Controller class name), humanize field labels (never the raw `Str::title(column)` like "Customer Id"), resolve every foreign key to the related record's human label via its relation (never echo the raw `*_id`), format money with `number_format`/currency, and use the project notification idiom (SweetAlert2 `Swal.fire`) instead of a native `alert`/`confirm`/`prompt`; and (b) include every `required_element` for a non-trivial view — extend the app layout and carry a responsive grid (`row` / `col-{sm,md,lg,xl}-*`) so the page works at 375px and desktop. This prose is defense-in-depth — the deterministic `validate-ui-quality.sh` runs on every view Write/Edit (PostToolUse) and the PreToolUse UI-quality gate (Branch 8) blocks the next `mega-sdd:execute-bolts` while any written view carries a scaffold tell or is missing a required element. The signatures are pack-declared (tech-agnostic); a stack that declares no `## UI quality signatures` section is exempt (the gate SKIPs).
...
**Parent-thread post-flight re-scan (IE-2 / FA-4 hardening).** The project-wide quality validators that scan GENERATED SOURCE/VIEWS — `validate-cross-cutting-registration.sh`, `validate-ui-quality.sh`, and (for vault edits) `validate-vault-oqs.sh` — fire via PostToolUse on the writer's Write/Edit. Under `--parallel` / `--per-squad`, the bolt subagent's writes are subagent-internal and invisible to the parent PostToolUse, so their state can be stale until a later parent-thread write re-triggers the project-wide rescan. To close that window: after each bolt batch completes, the parent thread (executing-plans) explicitly bash-invokes those validators against `$PROJECT_ROOT` so the gate state reflects current truth regardless of who wrote the files. This is the documented detect-and-block-next contract (a hook cannot un-write a mid-turn file); the explicit post-flight invoke makes "next" deterministic rather than dependent on an incidental parent write.
