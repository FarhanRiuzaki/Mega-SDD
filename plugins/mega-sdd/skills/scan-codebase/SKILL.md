...
10. **Write `codebase-map.md`** per `references/codebase-map-schema.md`. Include all sections; mark empty ones "None detected" (never omit). Stamp `engine` + `precision_tier` + `precision_downgrade_reason` (present ONLY when the Step 5 `--auto` lane downgraded the engine — the durable half of that lane's record) + `last_scanned_commit` (git HEAD; staleness stamp for drift detection) in frontmatter. **Step 10a — secret-scan gate:** scan the assembled map content for credential patterns BEFORE writing; redact matches as `[REDACTED-SECRET]`, then route the affected source `file:line` rows to `.mega-sdd/codebase/SECRET-FINDINGS.md` (durable rotation worklist — never the matched value) and also warn in chat (per `references/scan-procedure.md` Step 10a).
