...
|---|---|
| Step 0 (engine) | Run `scripts/probe-scan-engine.sh` — ONE spawn: D2 ladder digest (`ast-grep → regex`); ast-grep absent → `engine: regex` with a chat note. A forced `--engine=` whose binary is absent → `dep_missing` |
...
- `--engine=ast-grep|regex`: force a lane; default = `ast-grep → regex` via `scripts/probe-scan-engine.sh` (a `tree-sitter` value is rejected by the probe with a usage error)
