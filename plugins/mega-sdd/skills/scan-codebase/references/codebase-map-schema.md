...
`codebase-map.md` is the structured output of `scan-codebase`. It is consumed by `bind-codebase` to validate vault claims against repo reality. It is regenerable — never edited manually. It is ASSEMBLED by `scripts/derive-codebase-map.sh` (the model supplies a delta; carry-forward, the sha column, §1, the staleness stamp, and the write-path gates are script-owned — `scan-procedure.md` Step 10). Hand-written fixture maps remain valid inputs.
