...
- The user has an existing vault (generated by `generate-intent`) **and** an updated PRD/BRD that revises the source the vault was built from.
...
- The vault doesn't exist yet — use `generate-intent` for initial generation.
...
Run the same extraction logic as `generate-intent` Step 2 — build internal model of new source's components, entities, flows, decisions, constraints, gaps. Persist the four design-system flags (`HAS_UI_COMPONENTS`, `HAS_TOKENS`, `HAS_A11Y`, `HAS_VOICE_BRAND`) for the new source so design-system sections can also diff.
...
- **Vault doesn't exist** → STOP, suggest `generate-intent` for initial generation.
...
- **Multiple new source files conflict with each other** (e.g., new PRD says X, new tokens.json says Y for the same value) → emit conflict per existing source-merge rules in `generate-intent` Step 2. User resolves.
...
- Source vault: must be a `mega-sdd` vault with the standard 7-file structure, Vault Lock Status, OQ tagging convention, and Changelog.
