...
Source must be a `mega-sdd` vault with the standard structure (layout-2 4-file or legacy 7-file), the lock values, OQ tagging convention, and Changelog. OQ tagging conventions, status-marker semantics, and `vault.json` field rules + regeneration triggers: `../generate-intent/references/vault-core.md` (§OQ-conventions, §schema). OQ resolution mechanics (how `[x]` markers + resolution pointers are formatted): `resolve-oq` SKILL.md. Initial vault generation: `generate-intent`. Vault-vs-live-code reconciliation (not source-vs-source): `detect-drift`.
