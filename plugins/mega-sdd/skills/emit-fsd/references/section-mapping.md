...
**Source:** the vault Overview (`vault.md ## Overview`; legacy `01-overview.md`) §Goals/§Success criteria + §Non-Goals/§Out of Scope (both vocabularies accepted); layout-3: the PRD sections `<prd> §<heading>`, quoted verbatim
**Extraction:** Per §Goals: extract bulleted/numbered list as-is. Per §Non-Goals: same.
**Citation:** inline footnote per sub-section (layout-3: `<prd> §<heading>` (L<from>-L<to>)).
...
2. `<vault>/vault.json.stakeholders[]` (when populated; layout-3: `context.md` frontmatter `stakeholders:` — names only, PRD-sourced — reaches the builder here as plain strings, rendered `| Stakeholder | <name> | |`)
...
- For each unit + matching bolt-report:
  ...
  - Bolt commit: the line-anchored `commit:` / `bolt_commit:` field of bolt-report.md, else the sha on its `SDD-PROVENANCE:` line (ADV-012 — never a loose hex match, never `git log`)
