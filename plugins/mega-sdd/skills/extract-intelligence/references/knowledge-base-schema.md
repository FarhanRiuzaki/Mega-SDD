...
- **`mega-sdd:generate-intent` (Mode B with `--kb`)**: reads `README.md` + relevant domain files as PRD-equivalent source quotes. `[VERIFIED]` items → vault body; `[INFERRED]` → confirmation prompt; `[OPEN]` → vault OQ. When a workflow domain carries a §3a `stages:` block, generate-intent copies it **verbatim** into the matching `flows.md` flow, emits the corresponding Mermaid `stateDiagram`, and stamps the flow with `_kb_source: [20-workflows/<file>.md]` (the back-reference `validate-vault-flow-staging.sh` follows to prove staging was not dropped).
