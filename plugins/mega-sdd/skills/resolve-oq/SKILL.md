...
- **The derive run is mandatory** after each Resolve / Out-of-Scope / Defer outcome's markdown edits: **Run** `bash <plugin>/scripts/derive-vault-json.sh --vault <VAULT_DIR> --event '{"event":"oq-resolved|oq-deferred|oq-out-of-scope","id":"OQ-XXX","at":"<iso>","action":"A|B|C"}'` (+ `--patch <tmp-patch>` for binding-defers — a temp FILE whose content is `{"open_questions":{"OQ-XXX":{"defer_to":"binding"}}}`; `--patch` never takes inline JSON). The script recomputes status/summary/adrs/entities from the markdown and holds the `vault.json.lock` itself — exit 4 → surface the `memory_in_use` halt (keterangan unchanged); never hand-edit vault.json.
