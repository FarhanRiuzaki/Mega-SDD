...
Interactive resolver for Open Questions in a vault generated by `generate-intent`. Walks through the OQ roll-up by priority (P1 → P2 → P3), captures stakeholder answers, marks resolved entries inline, and bumps the vault version + Changelog.
...
- The user has a vault generated by `generate-intent` and wants to walk through the Open Questions list interactively.
...
- The vault hasn't been generated yet — use `generate-intent` first.
...
2. **Verify integrity**:
   ...
   - If any check fails → STOP, surface the issue. Suggest user run `generate-intent` first if vault is malformed/missing.
...
- **Vault not found / malformed** → STOP, surface the issue. Do not generate a vault from scratch (that's `generate-intent`'s job).
...
- Source vault: must be a `mega-sdd` vault with the standard 7-file structure and OQ tagging convention (`OQ-{DOC_CODE}-{N}` with `P1|P2|P3` priority).
- OQ tagging conventions, status marker semantics, and `vault.json` field rules: see `../generate-intent/references/vault-contract.md` (§OQ-conventions, §schema).
