...
The vault.md §Phase context block that consumes these values is specified in the generation guide (routed from the SKILL router).
...
| `[VERIFIED][INTENT]` | Outcome goal — state transition + business rule preserved; implementation references rebuild proposal | `vault.md ## Architecture` (rebuild shape) + `flows.md` (outcome); tagged `mutability_source: kb_intent` |
...
| `99-rebuild-architecture/module-dependency-graph.md` | Carried as a vault pointer for unit decomposition — record the KB path as the vault.md frontmatter `kb_module_graph: <path>`; `generate-units` reads it as a module-grouping + dependency seed | `vault.md` frontmatter (pointer) |
