...
- `handoff_missing` — orchestrate-flow v3.2.0+, Iter 40: sub-skill exited but no handoff YAML at expected path (file does not exist OR is zero bytes). ALWAYS STOP. Resolution: inspect sub-skill chat output for crash logs; re-run sub-skill standalone to reproduce; report as skill-author bug if reproducible. Closes Iter 38 audit finding D3-001 (silent-failure path closure).
