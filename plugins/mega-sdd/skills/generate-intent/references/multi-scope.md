...
> Load when the PRD declares a `scopes:` block or `--scope=<id>` is passed.
...
> **Patch-lane:** `title` / `scope` / `scope_metadata` / `prd_sha256` / `prd_path_at_generation` are AUTHORED fields — supply them in the `--patch` JSON consumed by `scripts/derive-vault-json.sh` at Step 3.8 (never hand-write vault.json). The deriver carries them forward verbatim on every later derive; `prd_sha256` is the at-generation pin `diff-vault` compares against and is NEVER recomputed by the script.
