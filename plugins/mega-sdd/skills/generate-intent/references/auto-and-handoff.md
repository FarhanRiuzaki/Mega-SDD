...
- **PROJECT_SHAPE**: auto-confirm the inferred shape ALWAYS (spec 2026-09-10 App. F6b). When inference confidence is low, record `project_shape_confidence: low` + the alternatives considered in the vault.json patch and add one delivery-report line — never a prompt under `--auto` (the audit measured this as a 0–1 conditional stop on the happy path).
