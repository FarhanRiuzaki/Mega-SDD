...
Converts a free-text user brief into a structured `seed-PRD.md` ready for `generate-intent` to consume. Runs an adaptive Q&A elaboration loop (≤10 questions) to fill gaps the brief leaves open. Captures everything verbatim — no paraphrasing, no auto-fills.
...
Suggested next step:
- generate-intent <OUTPUT_DIR>/source/seed-PRD.md
