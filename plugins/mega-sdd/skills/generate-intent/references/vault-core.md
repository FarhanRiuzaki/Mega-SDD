...
phase_total: <int>    # total phases planned (legacy numbered-tree KB only — parsed from suggested-phasing.md `## Phase` heading count). PRD-kontrak KBs are always 1 (module = phasing unit). Default 1.
```

### Field rules

- `phase` + `phase_total`: REQUIRED. Defaults `phase: 1, phase_total: 1` for back-compat (greenfield + Mode A PRD-driven + single-phase Mode B). Mode B with a LEGACY numbered-tree `--kb` parses `<KB>/99-rebuild-architecture/suggested-phasing.md` for phase count; a PRD-kontrak KB has no phase lane (module = phasing unit) and always writes 1/1. Missing field on an older vault.json → treat as `phase: 1, phase_total: 1`.
...
- `generate-intent` Step 3.8 — initial derive, single run after constitution (3.4) / classifier (3.5) complete: `derive-vault-json.sh --vault <dir> --patch <authored-patch>` (metadata + classifier writebacks).
