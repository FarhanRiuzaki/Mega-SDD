...
Reads (open these on disk): the vault's md files — layout-2: 4, legacy: 7 — (+ `constitution.md` when present) at the given DIR → then EVERY named source file. For `coverage_gap` you MUST sweep the WHOLE source (Grep/Read it fully) against the vault — a gap is, by definition, the thing the seed cannot contain. **Two carve-outs, checked BEFORE raising findings:**
- **Scope/phase filter:** on a `--scope`/`--phase` run the on-disk source is the UNFILTERED corpus while the vault is deliberately filtered — read the vault.md Scope / "Sibling scopes" / §Phase block FIRST (legacy: 00-index.md); source sections belonging to a sibling scope or an out-of-phase domain are **OUT OF SCOPE for `coverage_gap`**, not gaps.
