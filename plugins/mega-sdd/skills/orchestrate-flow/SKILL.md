...
- `--sync`: force the Mode D maintenance chain (changed-set derivation — incremental scan on map-bearing projects, `scripts/derive-changed-paths.sh` on express-born — → drift → re-bind → unit reconcile) regardless of other inference — the `/mega-sdd:sync` front-door (per `references/routing-rules.md` §Mode D)
