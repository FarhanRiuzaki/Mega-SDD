...
3. **Resolution preflight** (per `references/chain-execution.md`). Run in order; each is default-on and falls through silently when not applicable:
   ...
   - **Plan/Act gating** — `--plan` → Plan mode FIRST (write `.plan-pending`, STOP for user review); `--plan-then-act` → two-phase; default → Act. (There is no automatic PATCH|MINOR|MAJOR iter classifier — tombstone in `references/chain-execution.md` §Iter classifier hooks — so gating is flag-driven.)
...
- `--plan` / `--act` / `--plan-then-act`: Plan/Act gating — flag-driven (no automatic iter classifier; see the Step-3 bullet)
