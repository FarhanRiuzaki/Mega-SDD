...
- `--advisor` / `--no-advisor`: forwarded VERBATIM to the bind hop (bind-codebase owns them — `--advisor` forces the 6.15.0 scope-gated advisor pass on a bind it would skip; `--no-advisor` skips it regardless; both → bind refuses with `bind_inputs_missing`/`flag_contradiction`)
