...
|---|---|---|---|
| `extract-intelligence` | completed \| halted (quality gate fails twice per `wave-dispatch-templates.md` §gate failures) | → `mega-sdd:generate-intent --kb=<kb> --auto` | `extract-intelligence/references/handoff.md` |
...
| `emit-agents-md` | completed \| halted (`user_authored_conflict` / `vault_not_found` / `vault_corrupt` / `greenfield_no_bind_context` / `memory_in_use`) | `type: chain_complete` — AGENTS.md is the pipeline terminal output | local copy in `emit-agents-md/SKILL.md` §Handoff emission |
