...
> **`--lean` profile (tranche E):** when `.mega-sdd/config.yaml` carries `profile: lean`, the state engine (`scripts/_lib/state_probes.py`, `derived.profile`) reports the profile and the Stop-hook analyze aggregate stays off — the engine and the orchestrator's §Auto-integrated diagnostics are one contract (the 2a lesson). No chain row changes; no gate reads the profile. (The former `--no-advisor` hop transform died with the phase-advisor's removal in v7.4.0.)
...
| **Lane lite** (`--lite` / config `lane: lite`) + starterkit detected + no vault + PRD present | **2-hop (v8 P2):** `plan <prd> --lite --mode=<existing\|new>` → `execute-bolts --all --lite` — no bind hop (JIT bind at dispatch, execute-bolts pre-flight 3.9), no handoff YAML between the hops (state re-derived from disk + `validate-preflight.sh --predictive --chain=execute-bolts`); `--mode=existing` when the repo carries code, `new` for a bare scaffold (never asked). A brief / KB input stays on the express chain (plan is Mode A only in P2). A plan-born vault (`context.md`) with no units → `plan <prd> --lite --regenerate` (position `lite_context_no_units`) |
...
| **Delta lane — ticket-scale quoted free-text + owned vault** (overlay only: the front door or explicit user intent hands over a chat brief; the sentence names an entity/flow an existing vault's OQ/heading surface owns (constraints.md + vault.md; legacy: the 00-index roll-up); NEVER fired from derived state alone) | `diff-vault --from-prompt "<brief>"` → on clean apply the chain continues per **§Delta lane detail** below. A `prd_revision` (file newer than vault) OUTRANKS this row — a real PRD revision wins over a chat sentence. |
...
- **Lane lite / layout-3 vault (v8 P3, 8.0 — `derived.lane: lite` or `context.md` present): the state engine renders `scripts/rebind-units.sh --cwd . --vault <vault> --paths=@<vault>/.sync-changed-paths.txt` → `plan --reconcile` → `execute-bolts --all --lite` in place of `bind-codebase --paths=@…` → `generate-units --reconcile` → `execute-bolts`.** The vault is bound PER UNIT (no whole-vault binding.md — the classic hops FATAL there by design: `bind_folded_into_bolts` / `units_folded_into_plan`); `sync-intersect.sh` and `derive-delta-paths.sh` read the union of `bolts/U-*/binding.json` anchors ∪ units' `## Anchors` on this layout (fail-closed stays: a layout-3 vault with NO per-unit binding at all → exit 2 → full JIT re-bind `rebind-units.sh --units=all`). `rebind-units.sh` exit 0 = nothing affected (in_sync for the hop), 4 = re-bound (read `gate` — a CONFLICT closes the gate for the affected units exactly as at dispatch), 2/3 = fail-closed → full JIT re-bind. The full-scan fallback on this lane is `rebind-units.sh --units=all`, never `bind-codebase`.
