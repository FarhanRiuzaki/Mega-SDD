...
| **Lane lite** (`--lite` / config `lane: lite`) + starterkit detected + no vault + PRD present | **2-hop:** `plan <prd> --lite --mode=<existing\|new>` → `execute-bolts --all --lite` — no bind hop (JIT bind at dispatch, execute-bolts pre-flight 3.9), no handoff YAML between the hops (state re-derived from disk + `validate-preflight.sh --predictive --chain=execute-bolts`); `--mode=existing` when the repo carries code, `new` for a bare scaffold (never asked). A brief / KB input stays on the express chain (plan is Mode A only). A plan-born vault (`context.md`) with no units → `plan <prd> --lite --regenerate` (position `lite_context_no_units`) |
