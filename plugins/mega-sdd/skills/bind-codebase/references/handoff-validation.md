...
- ✅ Binding freshness RECERTIFY (`binding_stale_recertify` — `binding_metadata.head` vs non-unit-attributed commits in `<head>..HEAD` ∩ binding.json anchor paths; out-of-pipeline commits only — bolt commits ride B1/B3; P0 v4.92.0, migration-safe ladder for legacy artifacts)
