...
2. **Extract C** = field set from the codebase-map (AST signature extraction — ast-grep — at `precision_tier: ast`; regex fallback = lower confidence — run `/mega-sdd:install-deps --tools=ast-grep` (zero-compilation; auto tier-1 since 5.31.0) then re-scan for AST precision).
