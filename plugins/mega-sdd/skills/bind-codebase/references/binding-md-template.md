...
Write `binding.md` using this template (schema source: `binding-contract.md`). The model writes **NO banner and NO enum legend** — both are static boilerplate injected post-write by `scripts/derive-binding-json.sh` PHASE 0 (bind Step 4.5; see §Authoring notes). Conflicts render as `### CONFLICT-N` detail blocks ONLY — the machine-read form is the only form (no summary table).
...
> **Do-not-hand-edit banner (stamped post-write by `scripts/derive-binding-json.sh` PHASE 0).**
...
> The derive script's PHASE 0 injects it in Step 4.5; it is idempotent and parser-invisible (`_lib/binding_md.py` reads no region the banner or legend occupies, so `binding.json` is byte-identical whether derived before or after stamping).

> **`[reason: <enum>]` Anchor-cell token (machine-closed `state_reason` grammar).**
...
> written by `resolve-oq --binding`. Prose containing the word "resolved" elsewhere
...
> `scripts/derive-binding-json.sh` PHASE 0 (single source) — the script stamps it as a
