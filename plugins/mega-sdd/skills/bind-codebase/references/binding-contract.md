...
This enrichment is **advisory** — omitting these fields on an active CONFLICT is tolerated (no validator checks them); it does NOT change the CONFLICT-blocking contract (which still blocks on `conflict > 0`, per the table above).
...
**Anti-halu rails:** carried-forward rows keep their original citations untouched (no re-stamping); bound-vault production rules are unchanged (no `bound/` while any conflict — fresh OR carried — is active).
...
`<vault>/bound/` (nested in the vault dir, beside `units/` and `bolts/`) is DERIVED by `scripts/make-bound.sh` (bind Step 5) from the vault docs + `binding.json` — never hand-written by the model. It is a copy of the vault's markdown docs (layout-2: `vault.md` / `model.md` / `flows.md` / `constraints.md`; legacy: the seven `0[0-6]-*.md` docs; `vault.json` is NOT copied) with two augmentations:
