...
- **Classic lane only** (layout-2 four-file / legacy vault). Under `--lite` / config `lane: lite`, or whenever a layout-3 `context.md` vault is present, the PreToolUse gate FATALs `bind_folded_into_bolts` — binding is JIT per wave into `bolts/U-XXX/binding.json` (sole writer `scripts/write-unit-binding.sh`); use `execute-bolts --all --lite`, or `scripts/rebind-units.sh --units=all` for a full audit.
...
- **Flags:** `--strict` (block on OQ too, not just CONFLICT), `--auto`, `--kb=<path>`, `--no-kb`, `--no-framework-pack`, `--framework-pack=<custom-path>`, `--strict-constitution`, `--paths=<csv|@file>` (claim-scoped re-bind for the sync lane — re-verdict ONLY claims whose anchors or vault sections intersect the changed paths; everything else carried forward per `references/binding-contract.md §Claim-scoped re-bind`; **active CONFLICTs are ALWAYS re-validated, never carried silently** — the gate sees the full verdict set either way), `--express` (claim-scoped retrieval lane: claims enumerate from the script-derived `claims-ledger.json` skeleton PLUS a mandatory model completeness sweep of the vault docs — the ledger is never the claim boundary — and evidence comes from symbol-index queries + targeted source Reads with ZERO codebase-map load; the verdict grammar, the CONFLICT gate, and Steps 2.5–2.11 apply identically — full procedure, fail-closed ladder, and honest-fallback rules → `references/express-bind.md`). **Lane default:** the chain injects `--express` on every bind hop (express spine default); a bare standalone invocation runs the classic map lane unless `--express` is passed.
...
1. **`--vault=<path>`, else the first positional arg** — highest precedence. The chain always pre-resolves this on the main thread (the chain engine — `orchestrate-flow/references/routing-rules.md` — renders `mega-sdd:bind-codebase <vault> --auto`), so branches 2–3 are reached only on a standalone invocation.
...
- **2.6 Tech-OQ auto-resolution (scan)** + **2.7 Tech-OQ decisions (recommend)** → tech OQs are DECIDED, never handed to a human: a scan with one match resolves; no match / several matches → the AI picks reuse-first and records it `(AI decision …)` with a real citation (never an invented anchor, never `[tech / blocking]`); `recommend` OQs arrive already decided — bind verifies their citations and lists them in `## AI Technical Decisions`, deciding any still open. Runs at every `classification_confidence` (it grades the category call, not the decision). A missing FACT or a pick that reads as business goes back to the stakeholder as `[business]`. → `references/oq-resolution.md`.
