...
> **Non-interactive by contract (fork-ready).** This skill **NEVER calls `AskUserQuestion`** and never prompts on any path — the former `--auto` behavior is the *only* behavior. Inputs resolve deterministically from `$ARGUMENTS`/CWD (§Inputs **Step 0**); an unresolvable, ambiguous, or out-of-glob-root required input emits the `bind_inputs_missing` blocker, never a prompt. Conflict resolution — the KEEP_VAULT / KEEP_CODE / DEFER / SPLIT choice and any vault patch it implies — belongs to `resolve-oq`; bind writes verdicts and re-derives them on the next re-bind. This contract holds whether the body runs inline or as a forked subagent. The `context: fork` frontmatter flip is deliberately NOT taken yet: it is gated on two interactive runs (Precondition 0 — the detect-drift token before/after per `plugins/mega-sdd/CLAUDE.md`; and the depth-2 `Agent` probe that the scope-gated default-on Step 2.12 phase-advisor pass depends on (when the gate says run)).
...
- **Vault path** (positional, required) — the vault directory (layout-2 4-file or legacy 7-file).
...
- **Flags:** `--strict` (block on OQ too, not just CONFLICT), `--auto`, `--kb=<path>`, `--no-kb`, `--no-framework-pack`, `--framework-pack=<custom-path>`, `--strict-constitution`, `--no-advisor` (skip the phase-advisor pass regardless of the Step 2.12 scope gate; still honored under `--auto`), `--advisor` (FORCE the phase-advisor pass on a bind the Step 2.12 scope gate would skip — the mirror escape hatch; passing both `--advisor` and `--no-advisor` is a contradiction: emit the `bind_inputs_missing` blocker with `reason: flag_contradiction` — refuse with an error, never guess), `--paths=<csv|@file>` (claim-scoped re-bind for the sync lane — re-verdict ONLY claims whose anchors or vault sections intersect the changed paths; everything else carried forward per `references/binding-contract.md §Claim-scoped re-bind`; **active CONFLICTs are ALWAYS re-validated, never carried silently** — the gate sees the full verdict set either way), `--express` (claim-scoped retrieval lane: claims enumerate from the script-derived `claims-ledger.json` skeleton PLUS a mandatory model completeness sweep of the vault docs — the ledger is never the claim boundary — and evidence comes from symbol-index queries + targeted source Reads with ZERO codebase-map load; the verdict grammar, the CONFLICT gate, and Steps 2.5–2.12 apply identically (including the 2.12 scope gate) — full procedure, fail-closed ladder, and honest-fallback rules → `references/express-bind.md`). **Lane default:** the chain injects `--express` on every bind hop (express spine default); a bare standalone invocation runs the classic map lane unless `--express` is passed.
...
**1. Load inputs.**
   ...
   2. **`--express` set → this step and Step 2's retrieval are replaced by `references/express-bind.md`** (ledger + symbol-index + targeted Reads, zero map load; falls back HERE loudly when the index or ledger is unavailable) — Steps 2.5–2.12 and 3–6 then continue below unchanged.
...
**2.12 — Phase-advisor pass (adversarial second-opinion; scope-gated default-on, `--no-advisor` skips).**

**Scope gate (P3, USER-DECIDED 2026-08-17 — evaluate FIRST, from the draft verdict set + the KB probe, before any bundle build).** The gate reads the **FULL draft verdict set — on a `--paths` claim-scoped re-bind that means fresh AND carried-forward claims alike** (a carried IMPLEMENTED claim keeps the advisor in scope; the counted skip line below is computed over the same full set Step 3 aggregates). Run the advisor iff **ANY of**: (1) **KB lane** — a knowledge base was resolved for this bind (probe hit or `--kb=`, and not `--no-kb`); (2) **≥1 draft CONFLICT** in the verdict set; (3) **≥1 non-NEW draft claim** — any Implementation-State-Map state ∈ {IMPLEMENTED, PARTIAL_*, UNKNOWN} (existing-code evidence that could be falsely CONFIRMED — even one such claim is enough to run; fail-safe beats savings). Otherwise the bind is **all-NEW** greenfield — no false-CONFIRMED candidate exists, nothing to contradict — and the pass is SKIPPED with the counted, auditable Step 6 provenance `advisor: skipped (scoped — all-NEW greenfield bind: conflicts=0, non_new_claims=0, kb=none)` (the zeros are ASSERTED over the full draft set — the line is an auditable claim that the skip condition held, and by construction a skip can only ever print zeros). `--advisor` forces the pass on a scoped-skip bind; `--no-advisor` skips regardless of the gate; both together → the `bind_inputs_missing` blocker (`reason: flag_contradiction`) — refuse with an error, never guess. Spec: `docs/superpowers/specs/2026-08-17-advisor-scope-gate.md`.

When the gate (or `--advisor`) says run: first **Run** `bash <plugin>/scripts/build-advisor-bundle.sh --vault <vault>` to derive `<vault>/.advisor-bundle.md` — the compact SEED (draft verdict+anchor set + the map/vault/KB **paths** + the map's sha256 + the expansion contract). Dispatch the `mega-sdd:phase-advisor` agent with `references/advisor-checklist.md` (binding focus) + **the bundle path + the codebase-map / vault / KB PATHS** — **NOT** the pasted file contents. The advisor has `Read`/`Grep`/`Glob`: it opens each cited anchor and **Greps the full on-disk codebase-map** for `missed_match` (evidence outside the bundle is in scope — the bundle is a seed, never the advisor's horizon). Materialize its findings INTO the verdict set BEFORE Step 3 so they are counted + written as canonical `### CONFLICT-NNN` headings in Step 4 (the exact token the Step 5 gate AND `validate-handoff-binding-units.sh` → `.validation-blockers.json` read):
...
- Evidenceless findings are dropped. Record the pass in the Step 6 audit log: `advisor: {model, findings: {high,med,low}}` OR `advisor: skipped` (`--no-advisor` / lean profile) OR `advisor: skipped (scoped — …)` (the counted scope-gate form from Step 2.12 — the two skip forms are DISTINCT on purpose: the audit trail must tell a chosen opt-out from a gate decision) OR `advisor: unavailable` (agent error — NEVER reported as clean). Full focus + materialization → `references/advisor-checklist.md` + `plugins/mega-sdd/references/advisor-findings-schema.md`.
