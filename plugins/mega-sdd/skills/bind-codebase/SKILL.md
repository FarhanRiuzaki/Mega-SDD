...
> **Non-interactive by contract (fork-ready).** This skill **NEVER calls `AskUserQuestion`** and never prompts on any path — the former `--auto` behavior is the *only* behavior. Inputs resolve deterministically from `$ARGUMENTS`/CWD (§Inputs **Step 0**); an unresolvable, ambiguous, or out-of-glob-root required input emits the `bind_inputs_missing` blocker, never a prompt. Conflict resolution — the KEEP_VAULT / KEEP_CODE / DEFER / SPLIT choice and any vault patch it implies — belongs to `resolve-oq`; bind writes verdicts and re-derives them on the next re-bind. This contract holds whether the body runs inline or as a forked subagent. The `context: fork` frontmatter flip is deliberately NOT taken yet: it is gated on two interactive runs (Precondition 0 — the detect-drift token before/after per `plugins/mega-sdd/CLAUDE.md`; and the depth-2 `Agent` probe that the default-on Step 2.12 phase-advisor pass depends on).
