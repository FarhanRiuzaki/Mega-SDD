...
`plan` fuses the three classic pre-code phases (`generate-intent` → `bind-codebase` → `generate-units`) into ONE model turn (spec `docs/superpowers/specs/2026-09-10-v8-fused-pipeline-design.md` §3 / App. C3): read the PRD once, write `context.md` (layout-3, one file), write the units from PRD + context, derive `vault.json`, run the validators, and ask the human ONCE. Binding does not happen here — **JIT bind runs at dispatch** (execute-bolts pre-flight 3.9 reads each unit's `## Claims`), so a brownfield unit carries claims (a contract), never verdicts.
...
- **Input:** a PRD / BRD file (Mode A). A free-text brief (`--from-prompt`) or a knowledge base (`--kb`) is NOT a plan input — route those to `generate-intent` (classic).
...
- `--vault=<dir>` — output dir; default `.mega-sdd/vaults/<slug>` (slug from `derive-plan-pins.sh`). Existing `context.md` there → refuse unless `--regenerate` (never silently overwrite; `--reconcile` below never rewrites `context.md`).
...
**5. Validate (scripts; halts are findings, never patched around).** **Run** in order: `validate-unit-spec.sh --cwd=<root>` (scans every unit under `.mega-sdd/vaults/*`; an optional `--vault=<name|dir>` only scopes the EXIT CODE to that vault — the state file stays project-wide) (halts `unit_underspecified` / `hard_rule_unparseable` / `prd_source_unresolvable`; `xs_body_advisory` = trim); `validate-flow-coverage.sh --cwd=<root>` (BLOCKING gate); `validate-sibling-consistency.sh --cwd=<root>` (project-wide — it takes NO `--vault=`; that arg is `exit 2` + `STATUS: ERROR`); **`validate-plan-coverage.sh --cwd=<root> --prd=<prd> --vault=<vault>`** → exit 1 = halt `plan_coverage_gap` naming the PRD headings with no unit `prd_source` and no OQ (add a unit, raise an OQ, or an explicit Out-of-scope heading — the gap is the finding). Then `derive-vault-json.sh --vault=<vault> --event='{"event":"units_generated","count":N}'`.
...
Downstream: `execute-bolts --all --lite` (JIT bind per wave, unit-level readiness). Classic equivalents: `generate-intent` → `bind-codebase` → `generate-units` (default lane, untouched). Side lanes: `resolve-oq` (walk the remaining OQs), `analyze`, `emit-*` (DOCS lane reads `context.md` + the PRD directly).
