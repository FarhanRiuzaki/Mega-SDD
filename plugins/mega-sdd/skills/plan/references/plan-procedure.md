...
6. `## Open Questions` — every gap from the table + every "not stated" from steps 2–4, each with the mandatory category bracket, priority, `[origin: context.md#<anchor>]` when it arose elsewhere. Then classify (`../generate-intent/references/vault-core.md §Auto-classifier heuristics`): business → `blocking`; tech → `scan` (needs `scan_query`) / `recommend` (needs recommendation + rationale + citations + fallback — never fabricated); no match → business/blocking/low; business wins ties. **Tech OQs are DECIDED here, never asked and never left for `resolve-oq`** (`vault-core.md §AI technical decisions` — pick order, the never-decided list): there is no bind phase after PLAN, so a `scan` question is probed NOW (manifest / `query-symbol-index.sh` / the file) and every tech OQ is written `[x]` + `→ **Resolved v<X.Y>** (AI decision, <date>): <pick>`, then listed in `## AI Technical Decisions` (omit the section when none). A technical question the AI truly cannot answer is a missing FACT → `[business]` with the reason; a "PRD says X but the repo does Y" question is `[business]` too (the CONFLICT class). **xs:** every P2 *business* OQ is written `**Deferred (plan)**: resurfaced after bolts` — born deferred, never asked (`project_scale: xs` only).
...
**Patch shape for `derive-vault-json.sh --patch`** (a scratch JSON file, never `vault.json` itself): `{"source_documents": [...], "design_system_flags": {...}, ["design_system": {...}], ["scope": ..., "scope_metadata": {...}], "open_questions": {"OQ-XX-N": {"scan_query"|"recommendation"|"rationale"|"scan_citations"|"fallback_if_wrong": ...}}}`. Do NOT put `status` / `resolution` / `resolved_by` in an OQ patch entry (they mirror the markdown — the deriver rejects them, exit 2), nor `prd_sha256` / `prd_path_at_generation` / `author` / `stakeholders` — on layout-3 the deriver mirrors them from the frontmatter (md wins). Exit 2 → fix the markdown (the deriver names the section); exit 4 → halt `memory_in_use`.
...
| 6 | `validate-vault-oqs.sh --cwd=<root> --file-path=<vault>/context.md --strict-tech` (AFTER row 5 — it reads the derived `vault.json`) | `oq_tech_undecided` → decide the tech OQ (Step 3.6) or re-tag a missing fact `[business]`; `oq_decided_business_signal` → re-open it, drop the `(AI decision …)` annotation, tag `[business]` (it joins the Step-6 ask when P1); `oq_tech_missing_mode` / `oq_scan_missing_query` / `oq_recommend_underspecified` → fix the entry / patch. Re-run rows 5–6 |

**Read every validator's exit code directly.** Run each on its own line and do not pipe it (`… 2>&1 | tail -N; echo $?` reports `tail`'s 0, not the validator's — a usage error then reads as PASS; live case: `ERROR: unknown arg` followed by `rc=0`). If you must pipe, take `${PIPESTATUS[0]}`. Every validator prints its verdict as a `STATUS:` first line on stdout, so a wrong invocation is visible even through a pipe.
...
**Who:** open OQs with `[P1]` + `[business]` (after Step 3.6 classification), plus L0 toolchain items the front door queued for this ask (the batched ask absorbs L0). Tech OQs are never asked — here or anywhere: Step 3.6 already decided them, and Step 5 row 6 fails the phase if one is still open.
...
**Apply:** answered → `→ **Resolved v<X.Y>** (plan, <date>): <answer>` on the OQ line (`[x]`; the version slot is what the deriver keys the answer on — never the `(AI decision …)` marker, that one is for Step 3.6 picks only), the citing units drop the `TBD:` and keep the id in `binding_refs`; deferred → `**Deferred (plan)**: <reason>`; out of scope → `[~]` + `→ Out of Scope <version>: <reason>`. Re-run `derive-vault-json.sh` (statuses/timestamps are script-stamped). Units whose body still says `TBD: OQ-…` for a blocking OQ stay valid units — execute-bolts blocks their dispatch through `binding_refs` (existing rail), nothing new.

## Step 7 — self-check table + present

Verdict table (chat, ≤8 rows): `screens / entities / flows / OQs (P1 business open / deferred / decided by AI) / units (create / verify / extend) / modules` — each `<contract> → <written>`; a mismatch is fixed before presenting. Then the `../generate-intent/references/self-check.md` checklist (anti-halu, readability, output mode, vault.json integrity) applied to `context.md`, and the unit-level checks (`prd_source` resolves, `context_source` anchors exist, xs diet). Present: vault path, counts, `project_scale`, top blocker OQs (with unit ids), ONE line for the AI's picks when any exist (`N keputusan teknis diambil AI (H di P1) — review: <vault>/context.md#AI-Technical-Decisions; override: resolve-oq single-oq <OQ-ID>` — information, never a question), validators' PASS lines, then `NEXT: /mega-sdd --resume` (→ `execute-bolts --all --lite`). Auto-render HTML fail-open.

## Headless behavior

When `AskUserQuestion` is unavailable (a `claude -p` run), Step 6 cannot fire: the P1 business OQs stay `[ ]` + `blocking`, the summary says so, and the units that cite them stay blocked at bolts. The skill never answers a **business** OQ on the user's behalf — a benchmark runner that does so does it OUTSIDE the skill and labels it (`[ASSUMED-BY-RUNNER]`). Tech OQs are unaffected by headless mode: they were decided at Step 3.6 and need no ask.
