...
**Who:** open OQs with `[P1]` + `[business]` (after Step 3.6 classification), plus L0 toolchain items the front door queued for this ask (the batched ask absorbs L0). Tech OQs are never asked here (`scan` resolves at bind, `recommend` carries its recommendation, `blocking` tech stays open for `resolve-oq`).
