...
# Claim id = C-<DOCCODE>-<NN>, per-doc ordinal in document order. Stability across
# vault edits is the same class as model-minted ids (none guaranteed); the --paths
# "vault regenerated -> full re-bind" fallback covers renumbering.
...
# ── 00-index: implementation mode (Vault Lock bullet, shared-lib normalized) ──
lock_vals = vault_md.parse_vault_lock(docs.get("00-index.md", ""))
for i, line in enumerate(docs.get("00-index.md", "").splitlines(), 1):
    if MODE_RE.match(line.strip()):
...
lib_entities = vault_md.parse_data_model(docs.get("03-data-model.md", ""), errors)
lib_flows = vault_md.parse_flows(docs.get("04-flows.md", ""), errors)
lib_adrs = vault_md.parse_adrs(docs.get("05-decisions.md", ""), errors)
mine = {
...
for cls in ("entities", "flows", "adrs"):
    if mine[cls] != theirs[cls]:
        errors.append(
            ...
            f"_lib/vault_md.py {theirs[cls]} — the line-aware pass diverged "
            f"from the shared grammar; fix derive-claims-ledger.sh, never the lib."
        )

if not claims:
    errors.append(
        "zero claims extractable — vault docs empty or grammar mismatch; "
        "bind --express cannot run from an empty ledger (use the standard "
        "bind lane, or fix the vault per the generate-intent templates)."
    )
...
if errors:
    for e in errors:
        print("FAIL:", e)
    print(
        "KETERANGAN: artefak tidak cocok dengan grammar mega-sdd — ledger "
        "di-derive dari dokumen vault; perbaiki markdown vault mengikuti "
        "template generate-intent lalu jalankan ulang. JANGAN menulis "
        "claims-ledger.json manual (anti-laundering)."
    )
    sys.exit(2)
