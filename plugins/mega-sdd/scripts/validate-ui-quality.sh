...
#   validate-ui-quality.sh --deferral --cwd=<project-root> [--quiet]
#     (--deferral = the UI-deferral postflight merged verbatim from the former
#      validate-ui-deferral.sh, v7 Fase 2 group 7 — closes the sanctioned
...
# ─── --deferral mode (merged validate-ui-deferral.sh — writes its own state) ─
if [ "$DEFERRAL" -eq 1 ]; then
