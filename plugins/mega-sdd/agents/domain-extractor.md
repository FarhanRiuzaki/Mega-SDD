...
2b. §2 extras: a rule with ≥3 independent conditions = a DECISION TABLE, not prose; every `[LOCKED]` BR gets ≥1 `AC-<BR-id>-n` acceptance line (golden-master oracle) or an explicit `blocked-by-OQ-<id>` — see the template §2.
