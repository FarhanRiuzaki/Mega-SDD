...
**Nothing else goes in the trailer.** The former `Implements claim:` / `Anchors consulted:` / `Hard Rules active:` lines were dropped in 8.0.2 (team feedback #4, MEASURED: the trailer was 33–70 % of every comment line the bolts produced, and no gate, script, or reviewer ever read those three lines — only the marker line is verified). Their information already lives where it is validated: claim ids in your bolt-report `claims:` (`- C-NNN "<claim text>"`, Rule 5 forbids ids in code comments anyway), the anchors in the unit spec, the Hard rules in the unit `## Hard rules` (the post-flight engine recomputes compliance from git/fs ground truth, never from a comment). Do NOT add those lines back, and do NOT expand the trailer with a description of the file.

**If the sha is absent from the block, write `Unit: U-XXX` alone** and say so in your report — never guess or back-derive it. The `Generated by mega-sdd execute-bolts` marker line is never omitted.

Post-flight scan VERIFIES presence of the marker line. Missing → halt `provenance_missing`.
