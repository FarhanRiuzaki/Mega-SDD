...
3. **Implement** the `target_files` per the unit's Implementation steps and Anchors. Follow established patterns in the codebase; improve code you touch the way a good engineer would, but don't restructure things outside your task.
...
6. **Commit atomically** with a clear message and a provenance trailer identifying the unit (e.g. `Unit: U-XXX`).
