...
1. **Hard rules are absolute.** Honor every constraint in the unit's `## Hard rules` section: `DO NOT modify <path>`, `DO NOT add new <manifest> dependencies`, `<glob> MUST follow <case> naming`, `function <name> MUST preserve signature: <sig>`, `file <path> MUST exist after bolt`. The mechanical forms are machine-validated before and after your work — the post-flight scan runs against your landed commit (prose directives — generic MUST/DO NOT lines — are reviewed by the panel and recorded as advisory, not machine-verified), and a violation blocks the whole pipeline until that commit is fixed or reverted. If a Hard rule blocks the task as written, STOP and report `BLOCKED` — never work around it.
...
3. **Stay in scope.** Touch only the `target_files` (per each file's `operation`: create / modify) — the authoritative list is the unit frontmatter `target_files:` inside your dispatch file; the pointer's whitelist line is a convenience copy that may be degraded to a count or dropped entirely. A `task_type: verify` unit is read-only — it must NOT create/modify/delete anything. A deterministic post-hoc observer (B3) diffs your COMMITTED paths against `target_files` **∪ sanctioned extras**; an escaped path blocks the pipeline with `whitelist_violation`. **The sanctioned extras include the acceptance test you write** — test paths (`tests/`, `test/`, `spec/`, `specs/`, `__tests__/`, `*.test.{js,jsx,ts,tsx}`, `*.spec.{js,jsx,ts,tsx}`, `*_test.go`, `test_*.py`, `*Test.php`) plus `.mega-sdd/`, `docs/mega-sdd/` and vault/bolt artifacts — any other test file name (e.g. `foo.test.py`, `foo.spec.rb`) must be listed in `target_files` or B3 flags it. **So: COMMIT the acceptance test even when the unit does not list it in `target_files`** — units often do not list it, and B3 does not flag it. Do NOT emit `scope_creep_detected` over the acceptance-test path, and never park a written test outside the repo to avoid committing it: that leaves acceptance evidence asserting something true only in a working tree (field case 2026-08-29 — a bolt burned a full dispatch on exactly this false premise). `scope_creep_detected` is for a NON-sanctioned path outside `target_files`.
...
6. **Comments explain WHY, never WHAT** (a style rule, not a gate). Write a comment ONLY when the code cannot say it: a non-obvious business rule and where it comes from, a workaround and the reason it exists, a regulatory constraint, an assumption invisible in the code, a side effect, thread-safety or an exception the name does not imply, a `TODO`/`FIXME` that points at an OQ or a unit. Do NOT write: docblocks that restate the signature (`getUser` "Gets the user", getters/setters/constructors, non-public members whose name already says it, overrides whose contract lives on the interface/base, `@param string $name the name`), narration of what the next lines do (`// Step 1: validate input`, `// loop over items`), test-step narration that repeats the assertion, or a header that repeats the file name. A reader who knows the language learns nothing from those; the next unit pays to re-read them. **That list is OPEN, not closed** — it names the shapes that show up most, and a comment that is not on it is still forbidden when it fails the delete test. **The delete test decides every case, listed or not:** delete the comment; a reader who knows the language, has seen the name, the signature and the types, and has read this file down to that line — do they lose information? Not lost → not written. **In doubt → not written.** **Minimality for the comments that pass:** write the shortest text that still answers "why" — one line when one line is enough; a full docblock only when a toolchain reads it (typed generics, an IDE contract, an API/OpenAPI generator, a type-checker stub) **or on public API that crosses a module/team boundary** (a caller who will never open this file) — and even then it states the contract the signature cannot (a retention window, a side effect, thread-safety, an exception the name does not imply), never the signature itself; never add tags that restate the signature (`@param` / `@return` / `@throws` whose types are already written) just because the block is open. The active pack's `## Code style` slice in your dispatch (when present) names THIS stack's doc-comment tool, who reads it, and what to skip — follow it over your training-data habit. **Comment language follows the surrounding code:** the language of this file's existing comments, then its siblings'; a codebase with none → English, like the identifiers; never two languages in one file. **Never strip** the mega-sdd provenance trailer (§Provenance trailer — gate-read), a `// source: <path>:<line>` anchor in an e2e spec (`build-uat-e2e --check` reads it), any citation/anchor comment a validator reads, or a license/compliance header the pack requires.
...
2. **Tests first when required.** If the unit carries a `type: test` / `type: render` acceptance test (or the superpowers `test-driven-development` technique skill is in play — `references/superpowers-bridge.md`): write the test, run it, and confirm it FAILS for the right reason before writing implementation. A `type: render` test for a detail/show view must factory-create the model, GET the route, assert 200, AND assert a real display field renders (not a bare route-200 smoke test).
...
5. **Run the acceptance tests.** They must pass. **Bounded probes:** any network/HTTP check you run yourself (a `curl` against a dev server, a smoke request) is bounded — `--max-time` ≤ 60 and `--retry` ≤ 3 (wget `--tries` ≤ 3); a server that is not running is a `NEEDS_CONTEXT` report naming the command and port, never a wait loop, and a `type: manual` acceptance entry is recorded as pending, never "verified" by polling (field: `--retry 40 --max-time 120` against a server never started stalled a bolt 72 min; the PreToolUse rail denies `--retry ≥ 4` while a bolt is in flight — spec `2026-09-16-bounded-network-probes.md`). The acceptance commands themselves are re-run by the controller under its own 120 s bound.
6. **Commit atomically** with the canonical bolt message: subject `<type>(U-XXX): <unit title>` (conventional-commit type, unit ID as the scope) and ALL THREE trailers — `Unit: U-XXX`, `SDD-PROVENANCE: mega-sdd/execute-bolts unit=U-XXX`, and `SDD-Acceptance: v5` (the B4 acceptance-contract key: the controller re-executes your acceptance tests after your commit and records hook-guarded evidence — this trailer is what obligates that evidence). The gates key on this identity; a differently-shaped commit is invisible to the audit gates and flagged by the bypass guard. Under a parallel wave, sibling bolts commit into the same branch concurrently: a transient `index.lock` failure on `git add`/`git commit` is CONTENTION, not a defect — wait a moment and retry the git command (a few bounded attempts) before considering any escalation; never report `BLOCKED` for a lock that clears on retry. **Stage by explicit pathspec, always** — `git add <path> [<path>…]` naming the files you changed (your `target_files` + the test files you created), then `git commit -m …`. Never `git add -A` / `--all` / `.`, never `git commit -a`, never `--amend`, `stash`, or `reset --hard`: a sibling bolt's half-written files live in the same tree, and the PreToolUse wave rail DENIES those verbs while any bolt is in flight (the deny names the units; the fix is the pathspec form).
...
```yaml
bolt_self_report:
  model_used: "<copied VERBATIM from your own system prompt's 'You are powered by the model named ...' line — deterministic evidence of which model actually ran; never guessed>"
  confidence: <0.0-1.0>
  certain_decisions:
...
## Provenance trailer (MANDATORY in every file you modify — TWO lines, never more)
...
Unit: U-XXX · provenance: <provenance_path from the block — the repo-relative path of this unit's bolts/U-XXX/dispatch-prompt.md>
```

Line 1 is verbatim and never omitted. Line 2 carries the unit id and the pointer to the persisted dispatch file, where every provenance value (vault sha256, claim ids + texts, anchors, active Hard rules) already lives validated — the trailer points at that record instead of copying it. **Nothing else goes in the trailer**: the former `Implements claim:` / `Anchors consulted:` / `Hard Rules active:` / `vault sha256` lines were dropped (MEASURED: the old trailer was 33–70 % of every comment line the bolts produced, and no gate, script, or reviewer ever read those lines — only the marker line is verified). Claim ids stay OUT of code comments (Rule 5) — they live in your bolt-report `claims:`. Do NOT add those lines back, and do NOT expand the trailer with a description of the file.

**If `provenance_path` is absent from the block, write `Unit: U-XXX` alone** and say so in your report — never guess or back-derive a path, a sha, a claim id, an anchor or a rule. The `Generated by mega-sdd execute-bolts` marker line is never omitted.

Post-flight scan VERIFIES presence of the marker line (first 30 lines). Missing → halt `provenance_missing`.
...
**Names do the explaining.** A comment that explains a name is a naming defect: booleans read as a question or a state (`isValid`, `hasPermission`, `canRetry`), methods start with the verb that names the action (`calculateTotalPrice`, `fetchActiveOrders`, `validateEmail`), collections are plural (`activeOrders`), and `data`, `temp`, `obj`, `handle`, `process`, `Manager` are not names unless the pattern is genuinely called that. When you feel the urge to comment WHAT, rename instead — the pack's `## Code style` slice carries this stack's idiomatic forms.
