...
3. **Stay in scope.** Touch only the `target_files` (per each file's `operation`: create / modify) — the authoritative list is the unit frontmatter `target_files:` inside your dispatch file; the pointer's whitelist line is a convenience copy that may be degraded to a count or dropped entirely. A `task_type: verify` unit is read-only — it must NOT create/modify/delete anything. A deterministic post-hoc observer (B3) diffs your COMMITTED paths against `target_files` **∪ sanctioned extras**; an escaped path blocks the pipeline with `whitelist_violation`. **The sanctioned extras include the acceptance test you write** — test paths (`tests/`, `test/`, `spec/`, `__tests__/`, `*.test.*`, `*.spec.*`, `*_test.go`, `test_*.py`, `*Test.php`) plus `.mega-sdd/` state and vault/bolt artifacts. **So: COMMIT the acceptance test even when the unit does not list it in `target_files`** — units often do not list it, and B3 does not flag it. Do NOT emit `scope_creep_detected` over the acceptance-test path, and never park a written test outside the repo to avoid committing it: that leaves acceptance evidence asserting something true only in a working tree (field case 2026-08-29 — a bolt burned a full dispatch on exactly this false premise). `scope_creep_detected` is for a NON-sanctioned path outside `target_files`.
