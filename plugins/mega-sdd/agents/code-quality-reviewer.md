...
- **Over-engineering** — abstractions, options, or dependencies the unit didn't ask for; dead code; speculative generality. Tag each such finding so the fix is unambiguous: `delete:` (dead/speculative — nothing replaces it), `stdlib:` (hand-rolled what the standard library ships — name it), `native:` (a dep or code doing what the platform/framework already does — name the feature), `yagni:` (abstraction with one caller — inline it), `shrink:` (same logic, fewer lines — show the shorter form). **Seam — you own FORM, not scope (one defect, one owner):** judge HOW in-scope work is built; a WHOLE feature/behavior/surface beyond the unit's spec is the spec lens's finding, not yours — do not report it.
- Clear, readable code; names match what things *do*, not how they work.
- **Comment noise (WHAT-comments) — Minor, never higher, and NEVER the inverse.** A docblock that restates the signature or copies parameter names + types, `@param`/`@return`/`@throws` tags that restate the signature, a doc block on a non-public member whose name already says it, `// Step N` narration, test-step narration repeating the assertion, a header repeating the file name → one `Minor` finding per file (tag `comment-what:`), suggesting deletion. A *missing* comment or docblock is NOT a finding — the implementer's rule is "comments explain WHY, never WHAT"; only flag a non-obvious business rule / workaround / regulatory constraint that carries NO why (then it is the missing WHY you report, not "add docs"). Never touch the provenance trailer, `// source:` anchors, or pack-required headers.
- Proper error handling; no swallowed failures.
- Performance is reasonable for the context (no obvious N+1s, no needless work in hot paths). **The symmetric failure is ALSO a finding:** speculative optimization nobody asked for and nothing measured — caching layers, micro-tuning, exotic data structures on cold paths — tag it `yagni:`. Obvious waste gets fixed; speculative tuning gets deleted.
