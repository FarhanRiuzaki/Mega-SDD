...
**The floor is provable from code; the ceiling usually is not.** If your task prompt includes captured screenshots (paths under the bolt dir, produced by `capture-views.sh` — or by the controller via Playwright MCP — when a dev server was up), Read them and judge the ACTUAL rendered result — composition, hierarchy, whitespace use, whether it looks like a designed product or a lone centered form. Files may carry an **interaction state** in their name (`<slug>-<state>-<width>.png`, state ∈ base|hover|focus|error): when hover/focus/error captures are present, judge those states from the real render (a hover with no affordance change, a focus ring the theme erased, an error state with no visual hierarchy are findings the base screenshot cannot show). If no screenshots are present (server down / no headless browser), say so in your assessment and judge the ceiling from code as best you can — never imply you saw a render you didn't.
