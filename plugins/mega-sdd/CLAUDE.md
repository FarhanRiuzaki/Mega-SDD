...
- **[HOOK-VALIDATE] artifact integrity validators** — deterministic scripts + state files (OVERWRITE-NOT-APPEND, current truth). Iter 67.6 ships slice 1 (binding→units OQ-IDs). **Iter 78 (v3.69.0) adds 7 code-delivery quality validators** — `validate-flow-coverage` (PreToolUse Branch 5), `validate-sibling-consistency` (Branch 7), `validate-cross-cutting-registration` (Branch 11), render-test via `validate-unit-spec` (Branch 6), `validate-ui-quality` (Branch 8), `validate-dispatch-prompt` (Branch 9), and operator-UX via `validate-vault-oqs` (Branch 10) — all tech-agnostic (signatures from the framework-convention pack via `scripts/_lib/resolve-framework-pack.sh`; SKIP when a pack omits a section) and fixture-verified against the `new-tradefinance-import` git history (`tests/fixtures/code-delivery/**`). See `plugins/mega-sdd/references/fork-a-recovery-map.md` for the full classification + slice roadmap and `docs/superpowers/specs/2026-06-01-sharpen-code-delivery-uiux-design.md` for the design. **Iter-78.1 invariants (do NOT regress — `docs/superpowers/audits/2026-06-02-e2e-integration-audit.md`):** (1) the two EXTENSION gates (render-test via `validate-unit-spec`, operator-UX via `validate-vault-oqs`) MUST gate on a precise `halt_type` COUNT, never `status==FAIL` — else their pre-existing checks would spuriously block bolts; any new issue type on them is non-blocking until a PreToolUse branch opts it into the filter. (2) `_universal.md` intentionally backstops only the reasoning PRINCIPLE (prose-only bodies, no machine-parseable signatures) — adding concrete signatures there would silently convert graceful SKIPs into runs on every stack. (3) Every code-delivery validator reads ALL stack-specifics from the pack via `_lib/resolve-framework-pack.sh` and uses `errors="replace"` on reads (a crash = silent fail-open). **Iter 79 (v3.70.0) adds 5 new validators + 5 extensions from the end-to-end intelligence audit (`docs/superpowers/audits/2026-06-02-intelligence-e2e/`):** new — `validate-fanout-parity` (PreToolUse **Branch 12**, sibling deliverable-richness parity — "LC is always the survivor"), `validate-ui-deferral` (PreToolUse **Branch 13**, bolt-report defers a `## UI contract`), `validate-preflight` (PreToolUse **Branch 0**, predictive fatal input-precondition check — F2 closure), `validate-kb-reengineering` (PostToolUse, KB Wave-5 synthesis presence). De-vacuoused + newly-wired — `validate-conflict-classification` (PostToolUse on binding write; previously wired to no hook and greped a structure the producer never emits). Extensions — `validate-sibling-consistency` gains a `flow_step:` applies_when operator (closes the af49ede inbox side-effect parity gap via the pack `inbox-surfacing` concern); `validate-flow-coverage` emits advisory `decomposition_altitude_high`; `validate-vault-oqs` flags `oq_misclassified_tech`; `validate-codebase-map` adds a `precision_tier` depth check; `validate-handoff-yaml` type-checks CONDITIONAL fields + the new typed `next_action.confidence` ∈ [0,1]. **Iter-79 invariants:** (4) Branches 12/13 are DEDICATED single-purpose validators so gating on `status==FAIL` is precise (no unrelated checks ride their state); Branch 0 preflight only blocks on GENUINELY-FATAL missing inputs (self-clears each invocation — no deadlock). (5) advisory signals (`decomposition_altitude_high`, `oq_misclassified_tech`, codebase-map depth, kb-reengineering) MUST NOT flip an existing gate's blocking `status` — they are telemetry/WARN only. **v3.71.0 (semantic-depth — staged-input walking skeleton) adds staged-input non-loss enforcement across the KB→vault boundary:** new `validate-vault-flow-staging` (PreToolUse **Branch 14**, BLOCKING) follows each `04-flows.md` flow's `_kb_source` back-reference (the OQ-ID-class stable-identifier propagation per `generate-intent/references/vault-contract.md §stages-propagation`, NOT fuzzy title-matching) and FAILs with `vault_flow_staging_drop` when the cited KB workflow has a `## 3a` `stages:` block the vault flow dropped — catching the "single-form bolt where the legacy was a multi-step wizard" regression; `validate-kb-flows` gains an ADVISORY `kb_flow_staging_missing` (separate `advisories[]` channel — never flips status), and `validate-vault-flow-staging` ALSO carries an advisory arm (`vault_flow_staging_missing`, WARN-only) for the DOMINANT flatten case where a flow shows the workflow signal but has NEITHER a stages block NOR `_kb_source` (the blocking arm needs `_kb_source`, so it cannot see wholesale-flatten or PRD-only flows). **Honest coverage:** the KB advisory + this vault advisory are the BROAD detectors; the `status==FAIL` BLOCK is the NARROW precise case (back-ref preserved but stages dropped). `/mega-sdd:enrich-semantics` (`scripts/enrich-workflows-staging.sh`) is the two-phase remediation that retro-fits staging onto an existing KB. **Semantic-depth invariants:** (6) `validate-vault-flow-staging` is a DEDICATED single-purpose validator → its `status==FAIL` gate is precise (Iter-79 #4); backward-compatible *by construction* (no KB / no `_kb_source` on a flow / KB had no stages → SKIP, so pre-staging vaults never trip it). (7) `stages:` is an OPTIONAL field everywhere — downstream consumers ignore unknown frontmatter, so adding it breaks no consumer; per-stage execute-bolts wizard decomposition (vertical, via `depends_on`) stays Fork-B-future, as do the conditional / role-matrix / transition-guard dimensions. Fixture: `tests/fixtures/iter77-semantic-depth/` (16/16 Fork-A incl. the Branch-14 hook-fire gate). **v3.72.0 (extract-intelligence deepening) adds one ADVISORY validator + deepened extraction reasoning — NO new PreToolUse branch this iter (keystone B1 is a runnable verdict; hard-block deferred to protect the hook invariants above):** new `validate-extraction-scorecard.sh` checks the Wave-5 `.extraction-scorecard.json` (Extraction Completeness Contract) — SKIP when absent (back-compat), PASS when consistent, FAIL on internal inconsistency OR a hidden gap (a PARTIAL/MISSING principle with ZERO `[OPEN]` markers); `bind-codebase` consults it as a non-blocking preflight. The deeper extraction itself (P1–P4 disciplines wired into the wave-subagent dispatch prompt's `DEEP DISCIPLINES` block + the §3a progressive-disclosure schema enrichment) is **design-vocabulary / AI-coding-prompt scaffolding** — its WIRING presence is fixture-asserted (`tests/fixtures/iter80-extract-deepening/`), but the LLM actually performing the reasoning is best-effort Fork-B real-run behavior, NOT enforced. **Extract-deepening invariant:** (8) `validate-extraction-scorecard` is advisory (separate state file, not a hook branch) — it never blocks a tool call; B1 hard-block + B2/B3/E1/E2/E3 stay Fork-B-future until each has its own validator + fixture; §3a sub-fields are OPTIONAL (no consumer breaks).
