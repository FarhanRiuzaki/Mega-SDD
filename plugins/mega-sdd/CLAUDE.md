...
- **Commands** (`commands/`) — since 5.0.0 the public surface is THREE verbs: `/mega-sdd` (front door), `/mega-sdd:sync`, `/mega-sdd:emit <prd|fsd|sit|uat>`, plus the four maintenance one-timers (`migrate-paths`, `install-deps`, `update-plugin`, `memory`). Every other command file is a **deprecation alias** that keeps resolving (same skill, flags pass through, one-line keterangan first). **Never delete a pipeline command in a cull** — a pipeline command may be demoted to an alias only in a MAJOR release, must keep resolving for that whole major cycle, and may be removed only in the FOLLOWING major after telemetry review (v5 spec decision 2).
