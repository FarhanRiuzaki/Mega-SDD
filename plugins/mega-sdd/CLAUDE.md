...
**Commands** — the user's manual CLI entry points: three public verbs + four maintenance one-timers, and NOTHING else (the 5.x aliases were removed at 6.0.0 after telemetry review). **Never delete a pipeline command in a cull without the policy ladder** — demotion to an alias is allowed only in a MAJOR (the alias resolves for that whole major cycle); removal only in the following major, and only after telemetry review — and relocate any operative content into skill references BEFORE deletion (the 6.0.0 relocate-then-delete precedent, v6 spec §P4.1).
