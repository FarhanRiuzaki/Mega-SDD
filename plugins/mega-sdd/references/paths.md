...
- `<vault>/bolts/U-XXX/_claims.json` — the per-unit JIT capture a 3.9b re-bind writes (`rebind-units.sh --units` → `derive-unit-claims.sh --out=unit`): that unit's claims + the unit-scope dirty map. Script-written, gate-relevant (the binding records its sha256), Write/Edit-guarded; overwritten by the next re-bind. Track it with `bolts/` like the rest of the evidence.
- `<gitdir>/mega-sdd-freshness` (+ `.scope`, `.overlay`) and `<gitdir>/mega-sdd-seen` — the state-anchor view cache (spec `docs/superpowers/specs/2026-09-25-state-anchor-design.md` §5): written by the freshness engine (GROUND, the Stop bootstrap, `scripts/check-freshness.sh`) and read by session-start; the seen ring (`<session_id> <sha> <branch>`, ≤8 sessions) is written by session-start and UserPromptSubmit. They live in the per-worktree git dir (`.git/` or `.git/worktrees/<name>/`), so they can never be tracked, dirty the tree or need a `.gitignore` entry; deleting them costs one recompute.
