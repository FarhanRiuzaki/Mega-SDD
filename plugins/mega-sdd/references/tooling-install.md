...
|---|---|---|---|
| `tree-sitter` (or `tree-sitter-cli`) | scan-codebase OPT-IN lane only (`--engine=tree-sitter` — never auto since 5.31.0) | auto uses ast-grep (precision stays `ast`), else regex | macOS: `brew install tree-sitter-cli` · Linux/win: `cargo install tree-sitter-cli` · Node: `npm install -g tree-sitter-cli` |
| `ast-grep` (alias `sg`) | scan-codebase (TIER-1 AST extraction since 5.31.0 — zero-compilation, one spawn), execute-bolts, generate-units, detect-drift (Hard Rule v2 grammar) | scan falls to regex tier; v1-authored rules run natively; units carrying v2 rules need it installed | macOS: `brew install ast-grep` · Linux/win: `cargo install ast-grep` · Node: `npm install -g @ast-grep/cli` |
