...
- `citation_unresolvable` — emit-fsd: FSD.md cites a source path that does not resolve to an existing file (fabricated or stale citation; detected by `scripts/build-citation-map.sh`, exit 1). Details `{unresolved: [{section, cited_path}]}`. Resolution: correct the citation to a real artifact OR run the producing phase (scan-codebase / bind-codebase / generate-units / execute-bolts), then re-run emit-fsd.
