...
- **`validate-fsd-slots.sh` stays FSD-scoped — permanently.** It runs under `analyze` (run-analyze.sh family `fsd_slots`; the v7 PostToolUse scanner fan-out is gone), keys on the FSD path (`*FSD.md` / `*/fsd/*.md`) and writes `.mega-sdd/.fsd-slots-state.json`; widening the path filter (or adding a `--doc` flag) would change gate behavior for existing projects. **P5's chosen zero-risk wiring:** the prd/sit lanes run the engine's step-5 in-skill `grep -oE '\{\{[a-z0-9_-]+\}\}'` slot scan, and the SIT sign-off slot grammar is enforced by the SIBLING deterministic check `build-sit-evidence.sh --check-signoff` (run as a mandatory emit-sit gate step + as a re-emission guard) — no hook contract touched.
