...
All three scripts live in `plugins/mega-sdd/scripts/` and default to the FSD lane — **with `--doc` absent or `=fsd`, every code path is byte-identical to the pre-engine scripts** (the parity pin).
...
| `prd` | `emit-prd` (SKILL.md + references/prd-sections.md + prd-template.md) — forward + REVERSE (KB, no vault), `[VERIFIED]/[INFERRED]/[OPEN]` markers carried verbatim (`check-prd-markers.sh`) | LIVE |
