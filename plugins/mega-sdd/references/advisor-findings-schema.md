...
5. **Distinct provenance:** advisor-skipped, advisor-clean (0 findings), and advisor-unavailable (agent error/timeout) are distinct recorded states — a skipped/failed advisor is NEVER reported as "reviewed, no issues". The skipped state has TWO distinct recorded forms (6.15.0): `advisor: skipped` (chosen opt-out — `--no-advisor` / lean profile) vs `advisor: skipped (scoped — all-NEW greenfield bind: conflicts=0, non_new_claims=0, kb=none)` (the bind Step 2.12 scope-gate decision, counted over the FULL verdict set) — the audit trail must tell an opt-out from a gate decision, and the scoped form is the field counter for future advisor evaluation.
