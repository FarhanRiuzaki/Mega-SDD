...
Derived manifest (regenerated by `scripts/build-project-index.sh`, never hand-edited): one row per vault — slug, title, source doc, version, status (`intent` / `units-ready` / `in-progress` / `shipped`), unit + bolt counts, feature area. **The sequence of vaults IS the project's PRD/epic history.** A new vault reads this index to know what PRD 1..N-1 shipped, so it binds against the right reality. Regenerated at the end of any `generate-intent` / `execute-bolts` / `sync` chain.
