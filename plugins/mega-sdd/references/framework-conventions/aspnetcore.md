...
- **SQL injection** — query through EF Core LINQ or parameterized `FromSqlInterpolated` / `ExecuteSqlInterpolated` (or Dapper named parameters); `FromSqlRaw` / `ExecuteSqlRaw` (or a raw `SqlCommand`) built from an interpolated or concatenated user string is the injection bypass — `FromSqlInterpolated` parameterizes the interpolation for you. Base parameterization rule inherited from `dotnet.md`.
- **Secrets** — bind secrets through `IConfiguration` from User Secrets (Development), environment variables, or Azure Key Vault (`AddAzureKeyVault`); a connection string, API key, or token committed in `appsettings.json` or source is the leak. Base secrets rule inherited from `dotnet.md`.
