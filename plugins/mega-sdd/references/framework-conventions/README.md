...
Pluggable convention packs for common backend/frontend frameworks. Each pack declares file-location standards, naming standards, idioms, and Hard Rules that `bind-codebase` pulls into Suggested Unit Hard Rules when the matching framework is detected by `scan-codebase` (classic lane). On every lane the dispatch builder also filters the glob-matched pack rules into each bolt's T2 `framework_pack_rules` (`build-dispatch-prompt.sh` priority 7) and the pack's `## Code style` into `code_style_slice` (7b).

## Pluggable, not opinionated-by-default

mega-sdd stays framework-agnostic. Convention packs are **OPT-IN BY DETECTION**:

1. `scan-codebase` step 8.5 detects framework via package manifest fingerprints (`composer.json`, `package.json`, `Gemfile`, `pyproject.toml`/`requirements.txt`, `go.mod`, `Cargo.toml`, `pom.xml`/`build.gradle`, `*.csproj`)
2. `bind-codebase` step 2.8 loads matching pack from this folder
3. (classic lane) Pack's Hard Rules merged INTO `Suggested Unit Hard Rules` in `binding.md`
4. `execute-bolts` enforces the rules promoted into each unit's `## Hard rules` — v1 mechanical forms deterministically, v2 fenced ast-grep rules via `ast-grep scan`; unpromoted pack rules reach the bolt as advisory T2 `framework_pack_rules`
5. Lite / express-born run (no scan, no `bind-codebase`): the GROUND matcher (`state_probes.probe_framework_pack`) writes `state.json` `derived.framework_pack`; the resolver + dispatch builder read it — no `binding.md` exists there
...
| `<framework>.md` | One pack per framework. All 24 `pack_tier: full` packs lint clean — laravel, django, fastapi, next, express, nestjs, flask, symfony, rails, spring, nuxt, sveltekit, gin, slim, fastify, remix, sinatra, echo, fiber, actix, axum, rocket, aspnetcore, dotnet — plus the untiered project overlay `laravel-base-26` (25 rows in §8.5). See [`_registry.md`](_registry.md) for the authoritative readiness table (do not hand-edit). |
...
(classic lane — `bind-codebase` does not run under `--lite`; there the project pack root above is the override)
...
3. Test detection by running `scan-codebase` against a known project of that framework (classic); on a lite/express-born project check `state.json` `derived.framework_pack` (the GROUND matcher).
...
- Stale packs (>365 days `last_verified_against`) get an advisory note in `binding.md` at `bind-codebase` step 2.8 (classic lane, prose rule — no script checks the date)
