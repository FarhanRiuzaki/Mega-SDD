...
2. Fill in frontmatter (detection signature) + body sections, following the `<!-- REQUIRED -->` markers the scaffold leaves in place. Fill `## Code style (self-documenting)` with the stack's DELTA only (doc-comment tool + who reads it, skip/write vocabulary, naming examples) — every `read by` fact web-verified at authoring; REQUIRED since 8.2.0 (`_lint.md` Check 2 header + Check 6 shape; bullet count/byte cap pinned by `tests/per-stack-packs/test-code-style-section.sh`).
