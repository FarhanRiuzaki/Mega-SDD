...
## Comment conventions (universal default — a style rule, never a gate)

- A comment explains **WHY**, never WHAT: a non-obvious business rule + its source, a workaround + its reason, a regulatory constraint, an assumption the code cannot show, a side effect / thread-safety / an exception the name does not imply, a `TODO`/`FIXME` pointing at an OQ / unit.
- Forbidden (an OPEN list — the shapes that show up most, not a closed list; a comment that is not on it is still forbidden when it fails the delete test): docblocks that restate a signature or copy parameter names + types (`getUser`, `setStatus`, getters/setters/constructors, non-public members whose name already says it, overrides whose contract lives on the interface/base), line-by-line narration (`// Step 1: …`), test-step narration that repeats the assertion, file headers that repeat the file name.
- The delete test decides every case: delete the comment; a reader who knows the language, has seen the name, the signature and the types, and has read the file down to that line — do they lose information? Not lost → not written. In doubt → not written.
- Minimality for the comments that pass: the shortest text that still answers "why" — one line when one line is enough; a full docblock only when a toolchain reads it (typed generics, an IDE contract, an API/OpenAPI generator, a type-checker stub) or on public API that crosses a module/team boundary — and even then it states the contract the signature cannot, never the signature itself; never add tags that restate the signature (`@param` / `@return` / `@throws` whose types are already written) just because the block is open.
- Comment language follows the surrounding code: the file's existing comments, then its siblings'; a codebase with none → English, like the identifiers; never two languages in one file.
- Protected (load-bearing, never stripped): the mega-sdd provenance trailer (`Generated by mega-sdd execute-bolts …`, gate-read), `// source: <path>:<line>` anchors in e2e specs (`build-uat-e2e --check`), any citation/anchor comment a validator reads, license/compliance headers a pack requires.
- Per-stack packs carry their DELTA in `## Code style (self-documenting)` (the stack's doc-comment tool + who reads it, the skip/write vocabulary, idiomatic naming examples) — reaching the implementer as the T2 `code_style_slice`. Naming the tool is a WHY (a toolchain or a boundary-crossing caller needs it), never a licence to document every method. `_universal.md` itself carries no such section: this generic rule is agent-carried (Iron Rule 6).
- The review panel treats a WHAT-comment as a **Minor** finding and never reports "missing comment/docblock" as a finding.
