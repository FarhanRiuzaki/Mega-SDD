...
- Forbidden: docblocks that restate a signature or copy parameter names + types (`getUser`, `setStatus`, getters/setters/constructors), line-by-line narration (`// Step 1: …`), test-step narration that repeats the assertion, file headers that repeat the file name.
