...
- A comment explains **WHY**, never WHAT: a non-obvious business rule + its source, a workaround + its reason, a regulatory constraint, an assumption the code cannot show, a `TODO` pointing at an OQ / unit.
- Forbidden (an OPEN list — the shapes that show up most, not a closed list; a comment that is not on it is still forbidden when it fails the delete test): docblocks that restate a signature or copy parameter names + types (`getUser`, `setStatus`, getters/setters/constructors), line-by-line narration (`// Step 1: …`), test-step narration that repeats the assertion, file headers that repeat the file name.
...
- Per-stack packs may name their docblock tool (PHPDoc/JSDoc/docstring) for PUBLIC API surfaces the toolchain consumes (typed generics, IDE contracts) — that is a WHY (the type system needs it), not a licence to document every method.
