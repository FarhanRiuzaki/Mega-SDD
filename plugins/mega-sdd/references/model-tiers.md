...
4. **Field-pilot measurement (one command):** after ≥10 bolts on a pilot project, run `report-token-cost.sh --cwd=<root> --vault=<vault> --price-table=<gateway-prices.yaml>` — the `By bolt` table shows `model_used`/`escalated_from` per unit and the `Billed cost` section prices the run in gateway currency (per-MTok table; unpriced tokens are counted and flagged, never estimated). The 2026-08-22 pilot showed raw tokens are the WRONG flip metric (cheaper model ≠ fewer tokens) — decide on the billed number.
