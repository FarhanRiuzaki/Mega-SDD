...
- **Markers:** mutability `[LOCKED]` / `[INTENT]` / `[ARTIFACT]`; confidence `[VERIFIED]` / `[INFERRED]` / `[OPEN]`; OQ priority `P1` / `P2` + status `[~]` / `[ ]` / `[x]`; the AI-decision annotation marker `(AI decision, <date>)` on a resolved tech OQ — the parser derives `resolved_by: ai` from those exact English words, so a translated marker (`(Keputusan AI, …)`) silently drops the stamp (advisory from `validate-vault-oqs.sh --strict-tech`).
...
A human-facing question the human cannot answer from the prompt alone is a defect (user-mandated — a live run was blocked by a code-only prompt). EVERY interactive surface (AskUserQuestion, halt re-engagement, propose-and-confirm menu) MUST carry:
